...
function getSystemFont(size: number, nativeWeight: number, italic: boolean, symbolicTraits: number): UIFont {
    ...
    if (italic) {
        let descriptor = iosUtils.getter(result, result.fontDescriptor).fontDescriptorWithSymbolicTraits(symbolicTraits);
...
function createUIFont(font: Font, defaultFont: UIFont): UIFont {
    ...
    for (let i = 0; i < fontFamilies.length; i++) {
        ...
        if (shouldUseSystemFont(fontFamily)) {
            ...
            let actualItalic = iosUtils.getter(result, result.fontDescriptor).symbolicTraits & UIFontDescriptorSymbolicTraits.TraitItalic;
...
export module ios {
    export function registerFont(fontFile: string) {
        ...
        const fontData = iosUtils.getter(NSFileManager, NSFileManager.defaultManager).contentsAtPath(filePath);
        if (!fontData) {
