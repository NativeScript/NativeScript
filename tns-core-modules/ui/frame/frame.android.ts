...
import {
    _setAndroidFragmentTransitions, _onFragmentCreateAnimator, _onFragmentCreateAnimation, _getAnimatedEntries,
...
export class Frame extends FrameBase {
    ...
    public _navigateCore(newEntry: BackstackEntry) {
        ...
        if (newEntry.entry.prepareFragment) {
            newEntry.entry.prepareFragment(this.containerViewId, newEntry, this._currentEntry.fragment, newFragment, transaction, newFragmentTag);
        }
    ...
    public _popFromFrameStack() {
        ...
        if (this._android.hasOwnActivity) {
            (this._android.activity as any).supportFinishAfterTransition();
        }
    }
...
export function getFragmentClass() {
...
class FragmentCallbacksImplementation implements AndroidFragmentCallbacks {
    ...
    public onCreateAnimation(fragment: android.support.v4.app.Fragment, transit: number, enter: boolean, nextAnim: number, superFunc: Function): android.view.animation.Animation {
        ...
        let anim = _onFragmentCreateAnimation(this.entry, fragment, transit, enter, nextAnim, superFunc);
        if (traceEnabled()) {
            traceWrite(`${fragment}.onCreateAnimation(${transit}, ${enter ? "enter" : "exit"}, ${nextAnimString}): ${anim ? "anim" : "no anim"}`, traceCategories.NativeLifecycle);
        }
        return anim;
    }
