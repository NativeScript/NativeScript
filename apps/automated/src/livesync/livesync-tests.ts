...
export function setUp() {
	const labelPage = <Page>(
		Builder.createViewFromEntry({ moduleName: labelPageModuleName })
	);
...
function _test_onLiveSync_ModuleContext_AppStyle(
	appStyleFileName: string,
	livesyncStyleFileName: string
) {
	const pageBeforeNavigation = helper.getCurrentPage();
	const buttonPage = <Page>(
		Builder.createViewFromEntry({ moduleName: buttonPageModuleName })
	);
	...
	TKUnit.waitUntilReady(
		() =>
			pageAfterLiveSync.getViewById('button').style.color.toString() === green.toString()
	);
	...
	TKUnit.assertTrue(
		pageAfterLiveSync._cssState.isSelectorsLatestVersionApplied(),
		'Latest selectors version is NOT applied!'
	);

	helper.goBack();
	const pageAfterNavigationBack = helper.getCurrentPage();
	TKUnit.assertEqual(
		pageAfterNavigationBack.getViewById('label').style.color,
		green,
		'App styles NOT applied on back navigation!'
	);
	TKUnit.assertEqual(
		pageBeforeNavigation,
		pageAfterNavigationBack,
		'Pages are different'
	);
	TKUnit.assertTrue(
		pageAfterNavigationBack._cssState.isSelectorsLatestVersionApplied(),
		'Latest selectors version is NOT applied!'
	);
}
...
function _test_onLiveSync_ModuleContext(context: ModuleContext) {
	...
	TKUnit.waitUntilReady(
		() =>
			topmostFrame.currentPage &&
			topmostFrame.currentPage.isLoaded &&
			!topmostFrame.canGoBack()
	);
...
function _test_onLiveSync_ModuleReplace(context: ModuleContext) {
	...
	TKUnit.assertTrue(
		topmostFrame.currentPage.getViewById('button').isLoaded,
		'Button page is NOT loaded!'
	);
	...
	TKUnit.assertTrue(
		topmostFrame.currentPage.getViewById('label').isLoaded,
		'Label page is NOT loaded!'
	);
	TKUnit.assertEqual(topmostFrame.backStack.length, 0, 'Backstack is NOT clean!');
	TKUnit.assertEqual(
		pageBeforeNavigation,
		pageAfterBackNavigation,
		'Pages are different!'
	);
}

function _test_onLiveSync_ModuleContext_TypeStyle(
	styleModuleName: string,
	livesyncStyleFileName: string
) {
	const pageBeforeNavigation = helper.getCurrentPage();
	const buttonPage = <Page>(
...
function _test_onLiveSync_ModalViewClosed(context: ModuleContext) {
	const modalViewPage = <Page>(
		Builder.createViewFromEntry({ moduleName: modalViewPageModuleName })
	);
