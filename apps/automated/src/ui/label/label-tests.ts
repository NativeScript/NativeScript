...
export class LabelTest extends testModule.UITest<Label> {
	...
	public test_recycling() {
		helper.nativeView_recycling_test(() => new Label());
	}

	public test_Label_Members() {
		const label = new Label();
		TKUnit.assert(Utils.isDefined(label.text), 'Label.text is not defined');
		TKUnit.assert(Utils.isDefined(label.textWrap), 'Label.textWrap is not defined');
	}
	...
	public test_Set_BackgroundColor_TNS() {
		const label = this.testView;
		const expectedValue = new Color('Red');
	...
	public test_SetStyleProperties_via_css_class_Native() {
		...
		if (testLabel.android) {
			...
			expBackgroundColor = new Color(backgroundColor);
	...
	public test_SetStyleProperties_via_css_type_TNS() {
		...
		// << label-cssclass-type
		const expectedBackgroundColor = new Color(backgroundColor);
	...
	public test_BindingToText() {
		...
		const sourceModel = new Observable();
	...
	public test_BindingToText_WithBindingContext() {
		...
		const firstSourceObject = new Observable();
		...
		const secondSourceObject = new Observable();
	...
	public testLocalTextAlignmentFromCssWhenAddingCssFileAllSelectorsAreApplied() {
		...
		view.id = 'testLabel';
		page.addCss('#testLabel { text-align: ' + this.expectedTextAlignment + '; }');
		page.addCssFile(path.join(testDir, 'label-tests-page.css'));
