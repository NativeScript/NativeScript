...
export function registerNsModuleServerRoute(server: ViteDevServer, options: RegisterNsModuleServerRouteOptions): void {
	...
	// concurrent graph-walk fetch waves) skip the whole post-processing
		...
		try {
			...
			try {
				if (transformed?.code) {
					...
					try {
						if (strategy?.flavor === 'typescript' || strategy?.flavor === 'javascript' || strategy?.flavor === 'react') {
			...
			// ns:module `invalidateModules` + the runtime's eviction nonce,
