...
describe('websocket txn module builder', () => {
	...
	it('routes vue modules through /ns/asm and app modules through /ns/m (canonical, unversioned URLs)', () => {
		const code = buildTxnModuleCode(12, ['/app/main.ts', '/app/App.vue']);
		expect(code).toContain('await import("/ns/m/app/main.ts");');
		// Canonical assembler URL — the txn batch relies on the client's
		// prior eviction for freshness, not a version segment.
		expect(code).toContain('await import("/ns/asm?path=%2Fapp%2FApp.vue");');
	});
});
