...
export function rewriteImports(code: string, importerPath: string, sfcFileMap: Map<string, string>, depFileMap: Map<string, string>, projectRoot: string, verbose: boolean = false, outputDirOverrideRel?: string, httpOrigin?: string, resolveVendorAsHttp: boolean = false): string {
	...
	const replaceVueImport = (_match: string, prefix: string, spec: string, suffix: string): string => {
		...
		// `/node_modules/` segment in the path. Preserving the bare
		// spec routes them through the device import map's exact
		// `<pkg>` entry — the canonical `/ns/m/node_modules/<pkg>` URL
		// whose served body is the deps-bundle shim, i.e. the single
		// package instance. Rewriting here would mint a second URL
		// form for the same package. Subpath imports
		...
		if (spec && !spec.startsWith('/') && !spec.startsWith('./') && !spec.startsWith('../') && !/^https?:\/\//i.test(spec) && !spec.startsWith('@nativescript/core')) {
