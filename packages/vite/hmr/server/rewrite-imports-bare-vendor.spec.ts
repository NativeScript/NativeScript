 ...
 * the device-side import map dispatches it to the canonical
 * `/ns/m/node_modules/<pkg>` deps-shim URL (one decision point, alias
 * collapsing included) — mirroring the behaviour already used by the
 * existing `nodeModulesSpecifier` branch for paths that explicitly
 * contain `/node_modules/<pkg>/`.
 *
...
describe('rewriteImports: bare vendor specifier routing', () => {
	...
	it('preserves bare vendor specifiers (e.g. `pinia`) so the import map routes them', () => {
		// Reproduces the ConsumerApp boot crash: `pinia` is a CJS-shape
		// package. The device's import map routes the bare spec to the
		// deps-shim URL, which exposes the full named-export surface
		// (including `defineStore`). The rewriter must NOT inline its own
		// URL form — routing stays the import map's single decision.
		seedPackage('pinia');
	...
	it('preserves bare vendor specifiers even when packed onto a line with relative imports (App.vue scenario)', () => {
		...
		//      routes it to the canonical deps-shim URL.
		seedPackage('pinia');
		registerVendorManifest(
			makeVendorManifest({
				pinia: { id: 'pinia', exports: { '*': true } },
