 ...
 * mechanism primitives as the `ns:runtime` builtin module (reachable via
 * `require("ns:runtime")`, `import ... from "ns:runtime"` and
 * `import("ns:runtime")` — see the iOS runtime's docs/ns-builtin-modules.md);
 * every policy concern (boot orchestration, import.meta.hot, full reload,
 * CSS apply, WebSocket protocol) lives in JS inside @nativescript/vite.
 *
...
export interface NsRuntimeDevHostApi {
	/** `configureRuntime` — import map + volatile URL patterns + canonicalization vocabulary for the sync resolver. */
	...
	/** `terminateAllWorkers` — drains `Caches::Workers`; returns the count terminated (main realm only). */
...
/**
 * Resolve the dev host API from the `ns:runtime` builtin module via the
...
export function readNsRuntimeDevHostApi(target: Partial<typeof globalThis> = globalThis): NsRuntimeDevHostApi {
	...
	try {
		const api = requireFn('ns:runtime');
		return api && typeof api === 'object' ? (api as NsRuntimeDevHostApi) : {};
	} catch {
		// No such built-in module — a runtime predating ns:runtime or a
