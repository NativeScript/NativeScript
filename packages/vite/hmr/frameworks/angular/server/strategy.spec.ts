...
describe('angularServerStrategy', () => {
	...
	it('supplies no import-map entries', () => {
		expect(angularServerStrategy.importMapEntries).toBeUndefined();
	});

	it('owns the /@ng/component URL vocabulary (volatile + preserve-query)', () => {
		// `/@ng/component?c=<id>&t=<ts>` is volatile (each save mints a new
		// `t`, so the previous registry entry must be evicted on re-import)
		// AND must keep its query verbatim through canonicalization (`t` IS
		// the version discriminator `ɵɵreplaceMetadata` depends on). This
		// vocabulary lives here — the native runtime carries no framework
		// URL knowledge of its own.
