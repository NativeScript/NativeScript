...
import { sweepTrackedWorkers } from '../../../shared/runtime/worker-tracking.js';
...
// second cycle's `invalidateModules` (ns:module) ran, leaving the registry in
...
//
// `invalidateModules` is an ns:module builtin member that takes an
...
// evicts every loaded app module (ns:module `getLoadedModuleUrls` →
// `invalidateModules`) and re-imports the dev entry; the
...
function runHmrDisposeCallbacks(_g: any, options: { verbose?: boolean }): void {
	...
	// to run — verbose-only, like every other client log.
	if (executed && executed > 0 && options.verbose && envVerbose) {
		console.info(`[ns-hmr][dispose] ran ${executed} import.meta.hot.dispose callback${executed === 1 ? '' : 's'} before reboot`);
	}
}

// Terminate every live worker before Angular's reboot — fully userland.
//
// Drains `globalThis.__NS_HMR_WORKERS__`, the session-wide tracked set fed
// by the global `Worker` constructor interception the session bootstrap
// installs before any app/framework/vendor code evaluates
// (`shared/runtime/worker-tracking.ts`). Because the interception sits on
// the sole birth path for main-realm workers, the set covers every spawn —
// `new Worker(new URL(...))`, raw `Worker('~/x.js')`, dynamic-import- and
// plugin-spawned workers alike. Nested workers are reclaimed transitively:
// the runtime cascades `worker.terminate()` through each worker's children.
//
// Tolerant of:
//   * Missing global — non-worker apps have no Set; we no-op.
//   * `terminate()` throwing — the runtime can throw on already-dead
//     workers; the sweep counts per-entry failures so subsequent
//     terminations and the reboot itself still proceed.
//
// The Set is always cleared after the sweep so producer-side bookkeeping
// doesn't grow unbounded across HMR cycles.
function terminateTrackedWorkers(g: any, options: { verbose?: boolean }): void {
	const { terminated, failed, total } = sweepTrackedWorkers(g);

	// Surface ONE concise log per HMR cycle when there was actually work
	// to do — verbose-only, like every other client log: the dev terminal
	// stays quiet by default.
	if ((terminated > 0 || failed > 0) && options.verbose && envVerbose) {
		console.info(`[ns-hmr][workers] terminated ${terminated}/${total} tracked worker${total === 1 ? '' : 's'} before reboot`);
	}
}
...
function invalidateModules(urls: readonly string[], verbose: boolean): boolean {
	...
	if (typeof fn !== 'function') {
		console.warn(`[ns-hmr][client] runtime missing ns:module invalidateModules (runtime/vite version mismatch?); update cannot apply. evict=${urls.length}`);
		return false;
	}
	try {
		if (verbose && envVerbose) {
			console.info(`[ns-hmr][client] invalidateModules calling ns:module invalidateModules urls=${urls.length}`);
		}
		...
		console.warn('[ns-hmr][client] ns:module invalidateModules threw', (error as any)?.message || error);
		return false;
	}
}
...
export async function refreshAngularBootstrapOptions(msg: any, options: AngularUpdateOptions): Promise<void> {
	...
	// No client-side prefetch is needed after the eviction: the runtime's
	// async module-graph walk fetches the evicted modules concurrently on
	// background threads when the re-import below triggers it.
...
export async function handleAngularHotUpdateMessage(msg: any, options: AngularUpdateOptions): Promise<boolean> {
	...
	try {
		...
		if (typeof reboot === 'function') {
			...
			// Vite spec. The session-wide tracked set doesn't care
			// about module boundaries: the `Worker` constructor
			// interception recorded every spawn, and this sweep
			// terminates them all.
