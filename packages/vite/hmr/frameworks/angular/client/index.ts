...
// second cycle's `invalidateModules` (ns:runtime) ran, leaving the registry in
...
//
// `invalidateModules` is an ns:runtime builtin member that takes an
...
// evicts every loaded app module (ns:runtime `getLoadedModuleUrls` →
...
//
//   1. PREFERRED — `require('ns:runtime').terminateAllWorkers()` (NS iOS runtime
...
//      `terminateAllWorkers` on ns:runtime. It only catches workers spawned via
...
function terminateTrackedWorkers(g: any, options: { verbose?: boolean }): void {
	...
	if (nativeApi) {
		try {
			...
			if (options.verbose && envVerbose) {
				console.warn('[ns-hmr][client] ns:runtime terminateAllWorkers threw; falling back to JS-tracked Set:', (err as any)?.message ?? err);
			}
	...
	if (nativeApi && (nativeTerminated ?? 0) > 0) {
		console.info(`[ns-hmr][workers] terminated ${nativeTerminated} via the runtime's ns:runtime terminateAllWorkers before reboot`);
	} else if (!nativeApi && fallbackTerminated > 0) {
		console.info(`[ns-hmr][workers] terminated ${fallbackTerminated}/${fallbackTotal} via JS-tracked Set before reboot (ns:runtime terminateAllWorkers not exposed by this runtime)`);
	}
}
...
function invalidateModules(urls: readonly string[], verbose: boolean): boolean {
	...
	if (typeof fn !== 'function') {
		console.warn(`[ns-hmr][client] runtime missing ns:runtime invalidateModules (runtime/vite version mismatch?); update cannot apply. evict=${urls.length}`);
		return false;
	}
	try {
		if (verbose && envVerbose) {
			console.info(`[ns-hmr][client] invalidateModules calling ns:runtime invalidateModules urls=${urls.length}`);
		}
		...
		console.warn('[ns-hmr][client] ns:runtime invalidateModules threw', (error as any)?.message || error);
		return false;
	}
}
...
export async function handleAngularHotUpdateMessage(msg: any, options: AngularUpdateOptions): Promise<boolean> {
	...
	try {
		...
		if (typeof reboot === 'function') {
			...
			// (ns:runtime `terminateAllWorkers()`) doesn't care about module
