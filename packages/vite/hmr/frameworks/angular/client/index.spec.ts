...
// `ns:runtime` builtin module; install the test double once so member-level
...
describe('handleAngularHotUpdateMessage', () => {
	...
	it('still imports the stable canonical URL when the runtime lacks ns:runtime invalidateModules', async () => {
		const reboot = vi.fn();
		const importer = vi.fn(async () => ({}));
		const updater = vi.fn();
		const g = getGlobalScope();
	...
	it('survives ns:runtime invalidateModules throwing without breaking the import flow', async () => {
		const reboot = vi.fn();
		const importer = vi.fn(async () => ({}));
		const updater = vi.fn();
		const invalidator = vi.fn(() => {
...
//
//   1. PREFERRED: call ns:runtime `terminateAllWorkers()` (NS iOS
...
//      injects). Runtimes that don't expose ns:runtime `terminateAllWorkers`
...
describe('handleAngularHotUpdateMessage — worker termination before reboot', () => {
	...
	function snapshotWorkerGlobals(): SnapshotCleanup {
		const g = getGlobalScope();
		const previousReboot = g.__reboot_ng_modules__;
		const previousNative = nsRuntime.terminateAllWorkers;
		const previousSet = g.__NS_HMR_WORKERS__;
		const previousRegistry = g.__NS_HOT_REGISTRY__;
		return () => {
			g.__reboot_ng_modules__ = previousReboot;
			nsRuntime.terminateAllWorkers = previousNative;
			g.__NS_HMR_WORKERS__ = previousSet;
			g.__NS_HOT_REGISTRY__ = previousRegistry;
		};
	}

	it('prefers ns:runtime terminateAllWorkers() when the native API is present', async () => {
		...
		g.__reboot_ng_modules__ = reboot;
		nsRuntime.terminateAllWorkers = nativeTerminate;
	...
	it('falls back to draining __NS_HMR_WORKERS__ when no native API is present', async () => {
		...
		delete nsRuntime.terminateAllWorkers;
	...
	it('falls back to JS-Set drain when the native API throws (defensive)', async () => {
		...
		// regressed).
		nsRuntime.terminateAllWorkers = vi.fn(() => {
	...
	it('swallows per-worker terminate() failures so one bad worker cannot break the HMR cycle', async () => {
		...
		g.__reboot_ng_modules__ = reboot;
		delete nsRuntime.terminateAllWorkers; // force fallback path
