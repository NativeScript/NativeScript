...
// `ns:module` builtin module; install the test double once so member-level
...
describe('handleAngularHotUpdateMessage', () => {
	...
	it('hands the eviction set to the runtime and imports the stable entry URL', async () => {
		...
		const previousInvalidator = nsRuntime.invalidateModules;
		...
		nsRuntime.invalidateModules = invalidator;
		...
		try {
			...
			nsRuntime.invalidateModules = previousInvalidator;
			g.__NS_ANGULAR_HMR_REGISTER_ONLY__ = previousRegisterOnly;
		}
	});

	it('still imports the stable canonical URL when the runtime lacks ns:module invalidateModules', async () => {
		const reboot = vi.fn();
		const importer = vi.fn(async () => ({}));
		const updater = vi.fn();
		const g = getGlobalScope();
	...
	it('survives ns:module invalidateModules throwing without breaking the import flow', async () => {
		const reboot = vi.fn();
		const importer = vi.fn(async () => ({}));
		const updater = vi.fn();
		const invalidator = vi.fn(() => {
...
//
// The fix is fully userland: the session bootstrap patches the global
// `Worker` constructor (`shared/runtime/worker-tracking.ts`) so every
// main-realm spawn lands in `globalThis.__NS_HMR_WORKERS__`, and
// `terminateTrackedWorkers` sweeps that set before each reboot. The
// runtime deliberately exposes no native worker-teardown member; nested
// workers are reclaimed transitively by `worker.terminate()`.
//
// These tests pin the BEHAVIOUR of that sweep so a bootstrap that stops
// installing the interception (or a sweep regression) is caught by the
...
describe('handleAngularHotUpdateMessage — worker termination before reboot', () => {
	...
	it('drains __NS_HMR_WORKERS__ (the session-wide tracked set) before reboot', async () => {
	...
	it('does not throw when the tracked set is absent (non-worker apps)', async () => {
		const restore = snapshotWorkerGlobals();
		const reboot = vi.fn();
		const g = getGlobalScope();
...
describe('handleAngularHotUpdateMessage — import.meta.hot.dispose drain before reboot', () => {
	...
	it('calls dispose BEFORE the worker terminator (correct order is dispose → terminate → reboot)', async () => {
		...
		g.__reboot_ng_modules__ = reboot;
		g.__NS_HOT_REGISTRY__ = makeFakeHotRegistry({ runDispose });
		g.__NS_HMR_WORKERS__ = new Set([trackedWorker]);
