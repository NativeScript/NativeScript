...
// of the SFC artifact set (ns:runtime `invalidateModules`), exactly like `/ns/m`.
...
`;
	...
	importMapEntries(origin: string) {
		// `nativescript-vue` + `vue` resolve from the deps-bundle HTTP shims
		// (`/ns/m/node_modules/<pkg>`), the same single realm every other
		// vendor root lives in. Key order (nativescript-vue then vue) is
		// significant; the caller merges conditionally so an existing vendor
		// entry wins.
		return {
			'nativescript-vue': `${origin}/ns/m/node_modules/nativescript-vue`,
			vue: `${origin}/ns/m/node_modules/vue`,
		};
	},
