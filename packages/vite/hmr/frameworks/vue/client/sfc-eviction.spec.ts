...
// canonical (unversioned) URLs + explicit eviction via ns:module
// invalidateModules before re-import. These tests pin the artifact URL set
// and the evict-before-import ordering in loadSfcComponent.
...
// The fake ns:module module — tests mutate members on this object the same
// way the device runtime's module carries them.
const nsRuntime = installFakeNsRuntime(g);
...
describe('loadSfcComponent eviction protocol', () => {
	...
	beforeEach(() => {
		...
		previousInvalidator = nsRuntime.invalidateModules;
		nsRuntime.invalidateModules = invalidator as never;
	});

	afterEach(() => {
		if (previousInvalidator === undefined) delete nsRuntime.invalidateModules;
		else nsRuntime.invalidateModules = previousInvalidator;
	});
	...
	it('survives the eviction primitive throwing without breaking the load', async () => {
		nsRuntime.invalidateModules = vi.fn(() => {
