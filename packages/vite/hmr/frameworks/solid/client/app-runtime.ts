...
function installHmrRemount(rootModule: string, mount: (c: any) => void): void {
	...
	const remount = async (changed: string[]): Promise<void> => {
		...
		try {
			...
			// edits. Inline `ns:runtime` require — this module is served to the
			// device standalone, so it stays import-free.
			const g: any = globalThis;
			let invalidate: ((urls: string[]) => void) | undefined;
			try {
				invalidate = typeof g.require === 'function' ? g.require('ns:runtime')?.invalidateModules : undefined;
			} catch {
