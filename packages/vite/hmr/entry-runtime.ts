	...
	try {
		...
		const importMapPromise: Promise<void> = (async () => {
			if (typeof configureRuntime !== 'function') {
				// Re-resolve `ns:runtime` fresh — an earlier snapshot may have
			...
			if (typeof configureRuntime !== 'function') {
				...
				console.error('[ns-entry] ns:runtime configureRuntime is unavailable — the import map cannot be installed and bare-specifier imports WILL fail. Update @nativescript/ios to a runtime that provides the ns:runtime builtin module.');
				return;
			}
		...
		// (ns:runtime `setDevBootComplete`) so the runtime stops pumping the
