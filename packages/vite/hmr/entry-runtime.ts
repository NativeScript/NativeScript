...
export default async function startEntry(opts: EntryOpts) {
	...
	// Track every `Worker` construction before any app/vendor module can
	// evaluate (idempotent — the session-bootstrap path installs the same
	// patch). The HMR client sweeps the tracked set before framework
	// reboots; worker teardown is fully userland.
	installWorkerConstructorTracking(getGlobalScope(), { verbose: VERBOSE });
	...
	try {
		...
		let configureLoader = readNsRuntimeDevHostApi(globalThis).configureLoader;
		const g = globalThis;
		const importMapPromise: Promise<void> = (async () => {
			if (typeof configureLoader !== 'function') {
				// Re-resolve `ns:module` fresh — an earlier snapshot may have
				// been taken through a stale or stubbed scope.
				configureLoader = readNsRuntimeDevHostApi(globalThis).configureLoader;
			}
			if (typeof configureLoader !== 'function') {
			...
			try {
				...
				if (imapRes.ok) {
					...
					configureLoader({
						...
						// Server-owned URL vocabulary for the runtime's canonical-key
						// function; runtimes that predate the field ignore it.
		...
		// (ns:module `setDevBootComplete`) so the runtime stops pumping the
