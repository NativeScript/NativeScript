...
class ActivityCallbacksImplementation {
	...
	@profile
	public onRequestPermissionsResult(activity: any, requestCode: number, permissions: Array<String>, grantResults: Array<number>): void {
		if (Trace.isEnabled()) {
	...
	@profile
	public onActivityResult(activity: any, requestCode: number, resultCode: number, data: android.content.Intent): void {
		if (Trace.isEnabled()) {
...
export function setActivityCallbacks(): void {
	if (!activityInterface) {
		...
		activityInterface = new com.tns.NativeScriptActivity.Interface({
			...
			onRequestPermissionsResult(activity: any, requestCode: number, permissions: Array<String>, grantResults: Array<number>): void {
				const callbacks = (activity as any)._callbacks as ActivityCallbacksImplementation;
				if (callbacks) {
					callbacks.onRequestPermissionsResult(activity, requestCode, permissions, grantResults);
				}
			},
			onActivityResult(activity: any, requestCode: number, resultCode: number, data: android.content.Intent): void {
				const callbacks = (activity as any)._callbacks as ActivityCallbacksImplementation;
				if (callbacks) {
