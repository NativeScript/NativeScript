...
function rgbaToArgbNumber(
	...
	if (
		...
		return (
			Math.round(a * 0xff) * 0x01000000 + r * 0x010000 + g * 0x000100 + b
		);
	} else {
		return null;
	}
...
export function convertHSLToRGBColor(
	hue: number,
	saturation: number,
	lightness: number
...
export function parseColorKeyword(
	value,
	start: number,
...
const backgroundRepeatKeywords = new Set([
	"repeat",
	"repeat-x",
	"repeat-y",
	"no-repeat",
...
export function parsePercentageOrLength(
	text: string,
	start: number = 0
): Parsed<LengthPercentage> {
...
export function parseAngle(value: string, start: number = 0): Parsed<Angle> {
	...
	if (angleResult) {
		...
		return (angleUnitsToRadMap[value.unit] || ((_, __, ___) => null))(
			start,
			end,
			value.value
		);
...
const backgroundPositionKeywords = Object.freeze(
...
export function parseBackgroundPosition(
	...
	function formatH(
		align: Parsed<HorizontalAlign>,
		offset: Parsed<LengthPercentage>
	) {
	...
	if (keyword && backgroundPositionKeywords.has(keyword.value)) {
		...
		if (
			secondKeyword &&
			backgroundPositionKeywords.has(secondKeyword.value)
		) {
			...
			const secondLength =
				...
				parsePercentageOrLength(text, end);
			if (secondLength) {
				end = secondLength.end;
			}
			...
			if (
				...
				return {
					...
					value: {
						x: formatH(
							<Parsed<HorizontalAlign>>keyword,
							firstLength
						),
						y: formatV(
							<Parsed<VerticalAlign>>secondKeyword,
							secondLength
						),
					},
				};
				...
				return {
					...
					value: {
						x: formatH(
							<Parsed<HorizontalAlign>>secondKeyword,
							secondLength
						),
...
function parseArgumentsList<T>(
	text: string,
	start: number,
	argument: (value: string, lastIndex: number, index: number) => Parsed<T>
): Parsed<Parsed<T>[]> {
...
export function parseLinearGradient(
	...
	const parsedArgs = parseArgumentsList<Angle | ColorStop>(
		text,
		end,
		(text, start, index) => {
			if (index === 0) {
				...
				const angleArg =
					parseAngle(text, start) || parseDirection(text, start);
				if (angleArg) {
...
export function parseUniversalSelector(
	text: string,
	start: number = 0
): Parsed<UniversalSelector> {
...
export function parseSimpleIdentifierSelector(
	text: string,
	start: number = 0
): Parsed<TypeSelector | ClassSelector | IdSelector | PseudoClassSelector> {
	...
	const value = <
		TypeSelector | ClassSelector | IdSelector | PseudoClassSelector
	>{ type, identifier };
...
export function parseSimpleSelectorSequence(
	text: string,
	start: number
): Parsed<SimpleSelector[]> {
...
export class CSS3Parser {
	...
	private consumeAToken(): InputToken {
		...
		switch (char) {
			...
			case "\\":
				return (
					this.consumeAnIdentLikeToken() || this.consumeADelimToken()
				);
			case "0":
			case "1":
			case "2":
			...
			case "U":
				if (this.text[this.nextInputCodePointIndex + 1] === "+") {
					const thirdChar = this.text[
						this.nextInputCodePointIndex + 2
					];
					if (
						(thirdChar >= "0" && thirdChar <= "9") ||
						thirdChar === "?"
					) {
	...
	public consumeAListOfRules(): Rule[] {
		...
		while ((inputToken = this.consumeAToken())) {
			...
			if (
				(<InputTokenObject>inputToken).type ===
				TokenObjectType.atKeyword
			) {
				this.reconsumeTheCurrentInputToken(inputToken);
				const atRule = this.consumeAnAtRule();
				if (atRule) {
	...
	public consumeAnAtRule(): AtRule {
		...
		while ((inputToken = this.consumeAToken())) {
			if (inputToken === ";") {
				...
				(<InputTokenObject>inputToken).type ===
					TokenObjectType.simpleBlock &&
				(<SimpleBlock>inputToken).associatedToken === "{"
			) {
	...
	private consumeAComponentValue(): InputToken {
		...
		if (
			typeof inputToken === "object" &&
			inputToken.type === TokenObjectType.functionToken
		) {
		...
		do {
			...
			switch (nextInputToken) {
				case ")":
					...
					const end = this.nextInputCodePointIndex;
					funcToken.text =
						name + "(" + this.text.substring(start, end);
