...
export class ImageSource implements ImageSourceDefinition {
	...
	static fromResourceSync(name: string): ImageSource {
		const nativeSource =
			(<any>UIImage).tns_safeImageNamed(name) ||
			(<any>UIImage).tns_safeImageNamed(`${name}.jpg`);
	...
	static fromResource(name: string): Promise<ImageSource> {
		return new Promise<ImageSource>((resolve, reject) => {
			try {
				(<any>UIImage).tns_safeDecodeImageNamedCompletion(
					name,
					(image) => {
						if (image) {
							resolve(new ImageSource(image));
	...
	static fromFile(path: string): Promise<ImageSource> {
		return new Promise<ImageSource>((resolve, reject) => {
			try {
				(<any>UIImage).tns_decodeImageWidthContentsOfFileCompletion(
					getFileName(path),
					(uiImage) => {
	...
	static fromFileOrResourceSync(path: string): ImageSource {
		...
		if (path.indexOf(RESOURCE_PREFIX) === 0) {
			return ImageSource.fromResourceSync(
				path.substr(RESOURCE_PREFIX.length)
			);
		}
	...
	static fromFontIconCodeSync(
		...
		const attributes = {
			[NSFontAttributeName]: font.getUIFont(
		...
		const attributedString = NSAttributedString.alloc().initWithStringAttributes(
			source,
			<NSDictionary<string, any>>attributes
		);
	...
	public setNativeSource(source: any): void {
		if (source && !(source instanceof UIImage)) {
			throw new Error(
				"The method setNativeSource() expects UIImage instance."
			);
		}
		this.ios = source;
	}
