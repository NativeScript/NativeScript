...
function onRequestComplete(
	requestId: number,
	result: org.nativescript.widgets.Async.Http.RequestResult
) {
	...
	callbacks.resolveCallback({
		content: {
			raw: result.raw,
			toArrayBuffer: () =>
				Uint8Array.from(result.raw.toByteArray()).buffer,
			...
			toImage: () => {
				...
				return new Promise<any>((resolveImage, rejectImage) => {
					if (result.responseAsImage != null) {
						resolveImage(
							new imageSource.ImageSource(result.responseAsImage)
						);
					} else {
						rejectImage(
							new Error(
...
function buildJavaOptions(options: httpModule.HttpRequestOptions) {
	...
	if (options.headers) {
		const arrayList = new java.util.ArrayList<
			org.nativescript.widgets.Async.Http.KeyValuePair
		>();
