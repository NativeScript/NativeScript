...
const sessionTaskDelegateInstance: NSURLSessionTaskDelegateImpl = <
	NSURLSessionTaskDelegateImpl
>NSURLSessionTaskDelegateImpl.new();
...
export function request(
	...
	return new Promise<httpModule.HttpResponse>((resolve, reject) => {
		...
		try {
			...
			const dataTask = session.dataTaskWithRequestCompletionHandler(
				urlRequest,
				function (
					data: NSData,
					response: NSHTTPURLResponse,
							...
							content: {
								...
								toString: (encoding?: any) => {
									...
									if (typeof str === "string") {
										...
										throw new Error(
											"Response content may not be converted to string"
										);
									}
								},
								toJSON: (encoding?: any) =>
									parseJSON(NSDataToString(data, encoding)),
								toImage: () => {
									ensureImageSource();
									...
									return new Promise((resolve, reject) => {
										(<any>(
											UIImage
										)).tns_decodeImageWithDataCompletion(
											data,
											(image) => {
												if (image) {
													resolve(
								...
								toFile: (destinationFilePath?: string) => {
									...
									if (!destinationFilePath) {
										destinationFilePath = getFilenameFromUrl(
											options.url
										);
									}
