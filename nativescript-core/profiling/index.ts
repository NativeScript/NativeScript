...
export function uptime() {
	return global.android
		? (<any>org).nativescript.Process.getUpTime()
		: (<any>global).__tns_uptime();
}
...
function countersProfileFunctionFactory<F extends Function>(
	fn: F,
	name: string,
	type: MemberType = MemberType.Instance
): F {
	profileNames.push(name);
...
function timelineProfileFunctionFactory<F extends Function>(
	fn: F,
	name: string,
	type: MemberType = MemberType.Instance
): F {
	return type === MemberType.Instance
		? <any>function () {
				...
				try {
					...
					const end = time();
					console.log(
						`Timeline: Modules: ${name} ${this}  (${start}ms. - ${end}ms.)`
					);
				}
		  }
		: function () {
				...
				try {
					...
					const end = time();
					console.log(
						`Timeline: Modules: ${name}  (${start}ms. - ${end}ms.)`
					);
				}
		  };
}
...
let profileFunctionFactory: <F extends Function>(
	fn: F,
	name: string,
	type?: MemberType
) => F;
...
const profileMethodUnnamed = (target, key, descriptor) => {
	...
	descriptor.value = profileFunctionFactory(
		originalMethod,
		name,
		MemberType.Instance
	);
...
export function profile(
	nameFnOrTarget?: string | Function | Object,
	fnOrKey?: Function | string | symbol,
