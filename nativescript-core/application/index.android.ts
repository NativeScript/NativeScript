...
} from "../ui/frame/frame-interfaces";
...
export class AndroidApplication extends Observable
	implements AndroidApplicationDefinition {
	...
	private _registerPendingReceivers() {
		this._pendingReceiverRegistrations.forEach((func) =>
			func(this.context)
		);
	...
	public registerBroadcastReceiver(
		...
		const registerFunc = function (context: android.content.Context) {
			const receiver: android.content.BroadcastReceiver = new BroadcastReceiverClass(
				onReceiveCallback
			);
...
function initLifecycleCallbacks() {
	const setThemeOnLaunch = profile(
		"setThemeOnLaunch",
		(activity: androidx.appcompat.app.AppCompatActivity) => {
			...
			if (activityInfo.metaData) {
				const setThemeOnLaunch = activityInfo.metaData.getInt(
					"SET_THEME_ON_LAUNCH",
					-1
				);
	...
	const notifyActivityCreated = profile("notifyActivityCreated", function (
		activity: androidx.appcompat.app.AppCompatActivity,
	...
	const subscribeForGlobalLayout = profile(
		"subscribeForGlobalLayout",
		function (activity: androidx.appcompat.app.AppCompatActivity) {
	...
	const lifecycleCallbacks = new android.app.Application.ActivityLifecycleCallbacks(
		{
			onActivityCreated: <any>(
				profile("onActivityCreated", function (
					activity: androidx.appcompat.app.AppCompatActivity,
					savedInstanceState: android.os.Bundle
				) {
			...
			onActivityDestroyed: <any>(
				profile("onActivityDestroyed", function (
					activity: androidx.appcompat.app.AppCompatActivity
				) {
			...
			onActivityPaused: <any>(
				profile("onActivityPaused", function (
					activity: androidx.appcompat.app.AppCompatActivity
				) {
			...
			onActivityResumed: <any>(
				profile("onActivityResumed", function (
					activity: androidx.appcompat.app.AppCompatActivity
				) {
			...
			onActivitySaveInstanceState: <any>(
				profile("onActivitySaveInstanceState", function (
					activity: androidx.appcompat.app.AppCompatActivity,
			...
			onActivityStarted: <any>(
				profile("onActivityStarted", function (
					activity: androidx.appcompat.app.AppCompatActivity
				) {
					androidApp.notify(<AndroidActivityEventData>{
			...
			onActivityStopped: <any>(
				profile("onActivityStopped", function (
					activity: androidx.appcompat.app.AppCompatActivity
				) {
					androidApp.notify(<AndroidActivityEventData>{
...
function initComponentCallbacks() {
	let componentCallbacks = new android.content.ComponentCallbacks2({
		...
		onConfigurationChanged: <any>(
			profile("onConfigurationChanged", function (
