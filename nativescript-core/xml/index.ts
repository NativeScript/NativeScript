...
export class XmlParser implements definition.XmlParser {
	...
	constructor(
		onEvent: (event: definition.ParserEvent) => void,
		...
		const that = this;
		this._parser.on("startNode", function (
			elem,
			attr,
			uq,
			tagend,
			...
			if (attributes) {
				for (let key in attributes) {
					if (attributes.hasOwnProperty(key)) {
						...
						attributes[key] = XmlParser._dereferenceEntities(
							attributes[key]
						);
					}
				}
			...
			if (that._processNamespaces) {
				const stackEntry = XmlParser._getNamespacesStackEntry(
					attributes
				);
	...
	private _resolveNamespace(
		fullName: string
	): { prefix: string; namespace: string; name: string } {
