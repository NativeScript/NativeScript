import {
	Observable as ObservableDefinition,
	WrappedValue as WrappedValueDefinition,
} from ".";
...
export class Observable implements ObservableDefinition {
	...
	public setProperty(name: string, value: any) {
		...
		if (this.hasListeners(specificPropertyChangeEventName)) {
			const eventData = this._createPropertyChangeData(
				name,
				value,
				oldValue
			);
	...
	public removeEventListener(
		...
		for (let i = 0, l = events.length; i < l; i++) {
			...
			if (callback) {
				...
				if (list) {
					const index = this._indexOfListener(
						list,
						callback,
						thisArg
					);
	...
	public _createPropertyChangeData(
		propertyName: string,
		value: any,
		oldValue?: any
	): PropertyChangeData {
...
function addPropertiesFromObject(
	observable: ObservableFromObject,
	source: any,
