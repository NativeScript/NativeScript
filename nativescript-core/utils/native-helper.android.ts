...
export module ad {
	...
	export function dismissSoftInput(nativeView?: android.view.View): void {
		...
		if (nativeView instanceof android.view.View) {
			...
			const decorView = androidApp.foregroundActivity
				.getWindow()
				.getDecorView();
	...
	export module collections {
		export function stringArrayToStringSet(
			str: string[]
		...
		export function getPaletteColor(
			...
			try {
				...
				if (colorID) {
					...
					context
						.getTheme()
						.resolveAttribute(colorID, typedValue, true);
	...
	export function isRealDevice(): boolean {
		...
		return (
			fingerprint != null &&
			(fingerprint.indexOf("vbox") > -1 ||
				fingerprint.indexOf("generic") > -1)
		);
	}
}
