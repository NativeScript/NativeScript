...
function isOrientationLandscape(orientation: number) {
	return (
		orientation === UIDeviceOrientation.LandscapeLeft /* 3 */ ||
...
export module ios {
	...
	export const MajorVersion = NSString.stringWithString(
	...
	export function joinPaths(...paths: string[]): string {
		...
		return NSString.stringWithString(
			NSString.pathWithComponents(<any>paths)
		).stringByStandardizingPath;
	}

	export function getVisibleViewController(
	...
	export class UIDocumentInteractionControllerDelegateImpl extends NSObject
	...
	export function isRealDevice() {
		try {
			...
			const mediaTypes = UIImagePickerController.availableMediaTypesForSourceType(
				sourceType
			);
