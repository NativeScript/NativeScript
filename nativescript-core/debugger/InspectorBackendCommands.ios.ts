...
export namespace HeapDomain {
	...
	export interface HeapDomainDispatcher {
		...
		getRemoteObject(
			params: GetRemoteObjectMethodArguments
...
export namespace DebuggerDomain {
	...
	export interface DebuggerDomainDispatcher {
		...
		setBreakpointByUrl(
			params: SetBreakpointByUrlMethodArguments
		): { breakpointId: BreakpointId; locations: Location[] };
		// Sets JavaScript breakpoint at a given location.
		setBreakpoint(
			params: SetBreakpointMethodArguments
	...
	export class DebuggerFrontend {
		...
		paused(
			...
			reason: any /* XHR,DOM,EventListener,exception,assert,CSPViolation,DebuggerStatement,Breakpoint,PauseOnNextStatement,other */,
			data?: Object
		): void {
			__inspectorSendEvent(
				JSON.stringify({
...
export namespace RuntimeDomain {
	...
	export interface RuntimeDomainDispatcher {
		...
		parse(
			params: ParseMethodArguments
		): { result: SyntaxErrorType; message?: string; range?: ErrorRange };
		...
		callFunctionOn(
			params: CallFunctionOnMethodArguments
		): { result: RemoteObject; wasThrown?: boolean };
		...
		getCollectionEntries(
			params: GetCollectionEntriesMethodArguments
		): { entries: CollectionEntry[] };
		// Assign a saved result index to this value.
		saveResult(
			params: SaveResultMethodArguments
		): { savedResultIndex?: number };
		...
		getRuntimeTypesForVariablesAtOffsets(
			params: GetRuntimeTypesForVariablesAtOffsetsMethodArguments
		...
		getBasicBlocks(
			params: GetBasicBlocksMethodArguments
...
export namespace ConsoleDomain {
	...
	export interface ConsoleDomainDispatcher {
		...
		setMonitoringXHREnabled(
			params: SetMonitoringXHREnabledMethodArguments
		): void;
	...
	export class ConsoleFrontend {
		...
		heapSnapshot(
			timestamp: number,
			snapshotData: HeapDomain.HeapSnapshotData,
			title?: string
		): void {
			__inspectorSendEvent(
				JSON.stringify({
...
export namespace PageDomain {
	...
	export interface PageDomainDispatcher {
		...
		addScriptToEvaluateOnLoad(
			params: AddScriptToEvaluateOnLoadMethodArguments
		): { identifier: ScriptIdentifier };
		removeScriptToEvaluateOnLoad(
			params: RemoveScriptToEvaluateOnLoadMethodArguments
		): void;
		...
		getResourceContent(
			params: GetResourceContentMethodArguments
		): { content: string; base64Encoded: boolean };
		// Searches for given string in resource content.
		searchInResource(
			params: SearchInResourceMethodArguments
		): { result: GenericTypesDomain.SearchMatch[] };
		// Searches for given string in frame / resource tree structure.
		searchInResources(
			params: SearchInResourcesMethodArguments
		): { result: SearchResult[] };
		...
		setScriptExecutionDisabled(
			params: SetScriptExecutionDisabledMethodArguments
		): void;
		// Toggles mouse event-based touch event emulation.
		setTouchEmulationEnabled(
			params: SetTouchEmulationEnabledMethodArguments
		): void;
		...
		setCompositingBordersVisible(
			params: SetCompositingBordersVisibleMethodArguments
		): void;
		...
		handleJavaScriptDialog(
			params: HandleJavaScriptDialogMethodArguments
		): void;
...
export namespace NetworkDomain {
	...
	export interface NetworkDomainDispatcher {
		...
		getResponseBody(
			params: GetResponseBodyMethodArguments
		...
		loadResource(
			params: LoadResourceMethodArguments
		): { content: string; mimeType: string; status: number };
	}
...
export namespace DOMDomain {
	...
	export interface DOMDomainDispatcher {
		...
		performSearch(
			params: PerformSearchMethodArguments
		): { searchId: string; resultCount: number };
		// Returns search results from given <code>fromIndex</code> to given <code>toIndex</code> from the sarch with the given identifier.
		getSearchResults(
			params: GetSearchResultsMethodArguments
		): { nodeIds: NodeId[] };
		...
		resolveNode(
			params: ResolveNodeMethodArguments
		): { object: RuntimeDomain.RemoteObject };
	}
...
export namespace CSSDomain {
	...
	export interface CSSDomainDispatcher {
		...
		getInlineStylesForNode(
			params: GetInlineStylesForNodeMethodArguments
		): { inlineStyle?: CSSStyle; attributesStyle?: CSSStyle };
		// Returns the computed style for a DOM node identified by <code>nodeId</code>.
		getComputedStyleForNode(
			params: GetComputedStyleForNodeMethodArguments
		): { computedStyle: CSSComputedStyleProperty[] };
		// Requests information about platform fonts which we used to render child TextNodes in the given node.
		getPlatformFontsForNode(
			params: GetPlatformFontsForNodeMethodArguments
		): { fonts: PlatformFontUsage[] };
		// Returns the current textual content and the URL for a stylesheet.
		getStyleSheetText(
			params: GetStyleSheetTextMethodArguments
		): { text: string };
	}
