...
export class Request {
	...
	public responseReceived(
		response: inspectorCommandTypes.NetworkDomain.Response
	): void {
	...
	public requestWillBeSent(
		request: inspectorCommandTypes.NetworkDomain.Request
	): void {
		if (this._networkDomainDebugger.enabled) {
...
export class NetworkDomainDebugger
	implements inspectorCommandTypes.NetworkDomain.NetworkDomainDispatcher {
	 ...
	 */
	setExtraHTTPHeaders(
		params: inspectorCommandTypes.NetworkDomain.SetExtraHTTPHeadersMethodArguments
	): void {
		//
	}
	 ...
	 */
	getResponseBody(
		params: inspectorCommandTypes.NetworkDomain.GetResponseBodyMethodArguments
	): { body: string; base64Encoded: boolean } {
		const resource_data = resources_datas[params.requestId];
		const body = resource_data.hasTextContent
			? NSString.alloc()
	 ...
	 */
	setCacheDisabled(
		params: inspectorCommandTypes.NetworkDomain.SetCacheDisabledMethodArguments
	): void {
		//
	}
	 ...
	 */
	loadResource(
		params: inspectorCommandTypes.NetworkDomain.LoadResourceMethodArguments
	): { content: string; mimeType: string; status: number } {
		let appPath = NSBundle.mainBundle.bundlePath;
		let pathUrl = params.url.replace("file://", appPath);
		let fileManager = NSFileManager.defaultManager;
		let data = fileManager.fileExistsAtPath(pathUrl)
