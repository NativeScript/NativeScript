...
export class FileSystemAccess {
			...
			if (!exists) {
				...
				if (
					...
					if (onError) {
						onError(
							new Error(
								"Failed to create file at path '" + path + "'"
							)
						);
					}
	...
	public getFolder(
		...
		try {
			...
			if (onError) {
				onError(
					new Error("Failed to create folder at path '" + path + "'")
				);
			}
	...
	public getExistingFolder(
		...
		try {
			...
			if (onError) {
				onError(
					new Error("Failed to get folder at path '" + path + "'")
				);
			}
	...
	private exists(path: string): { exists: boolean; isDirectory: boolean } {
		...
		const exists = fileManager.fileExistsAtPathIsDirectory(
			path,
			isDirectory
		);
		...
		for (let i = 0; i < entities.length; i++) {
			try {
				...
				if (onError) {
					onError(
						new Error(
							"Failed to empty folder '" + path + "': " + ex
						)
					);
				}
	...
	public getLogicalRootPath(): string {
		...
		const resolvedPath = NSString.stringWithString(mainBundlePath)
	...
	public readTextAsync(path: string, encoding?: any) {
		...
		return new Promise<string>((resolve, reject) => {
			try {
				...
				reject(
					new Error(
						"Failed to read file at path '" + path + "': " + ex
					)
				);
			}
		});
	...
	public writeTextAsync(
		...
		return new Promise<void>((resolve, reject) => {
			try {
				...
				reject(
					new Error(
						"Failed to write file at path '" + path + "': " + ex
					)
				);
			}
		});
	...
	public writeTextSync(
		...
		try {
			...
			if (onError) {
				onError(
					new Error("Failed to write to file '" + path + "': " + ex)
				);
			}
		}
	}
	...
	private getKnownPath(folderType: number): string {
		const fileManager = NSFileManager.defaultManager;
		const paths = fileManager.URLsForDirectoryInDomains(
			folderType,
	...
	private deleteEntity(path: string, onError?: (error: any) => any) {
		...
		try {
			...
			if (onError) {
				onError(
					new Error(
						"Failed to delete file at path '" + path + "': " + ex
					)
				);
			}
		}
		...
		try {
			...
			try {
				...
				if (onError) {
					onError(
						new Error(
							"Failed to enum files for folder '" +
								path +
								"': " +
								ex
						)
