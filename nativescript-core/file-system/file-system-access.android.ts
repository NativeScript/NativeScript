...
export class FileSystemAccess {
		...
		try {
			...
			if (!javaFile.isFile()) {
				if (onError) {
					onError({
						message:
							"The specified parameter is not a File entity.",
					});
				}
		...
		try {
			...
			if (!javaFile.getCanonicalFile().isDirectory()) {
				if (onError) {
					onError({
						message:
							"The specified parameter is not a Folder entity.",
					});
				}
					...
					new org.nativescript.widgets.Async.CompleteCallback({
						onComplete: (result: string) => {
							if (actualEncoding === textModule.encoding.UTF_8) {
								...
								result = FileSystemAccess._removeUtf8Bom(
									result
								);
							}
							resolve(result);
	...
	public readTextSync(
		...
		try {
			...
			const reader = new java.io.InputStreamReader(
				stream,
				actualEncoding
			);
