...
export class XMLHttpRequest {
	...
	private _loadResponse(r: http.HttpResponse) {
		...
		const mimeType =
			(contentType && contentType.toLowerCase()) || "text/xml";
		...
		if (this._responseType === XMLHttpRequestResponseType.json) {
			...
			this._responseType === XMLHttpRequestResponseType.arraybuffer
		) {
	...
	public abort() {
		...
		if (
			(this._readyState === this.OPENED && this._sendFlag) ||
...
export class File extends Blob {
	...
	constructor(
		...
		this._lastModified = opts.lastModified
...
export class FileReader {
	...
	public readAsDataURL(blob: Blob) {
		...
		this._result = `data:${blob.type};base64,${this._array2base64(
			Blob.InternalAccessor.getBuffer(blob)
		)}`;
	}
	...
	public readAsText(blob: Blob) {
		...
		const textDecoder = new TextDecoder();
		this._result = textDecoder.decode(
			Blob.InternalAccessor.getBuffer(blob)
		);
	}
