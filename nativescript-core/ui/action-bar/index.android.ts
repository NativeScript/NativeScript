...
export class AndroidActionBarSettings
	implements AndroidActionBarSettingsDefinition {
	...
	public _updateNavigationButton() {
		...
		if (navButton && isVisible(navButton)) {
			...
			if (systemIcon !== undefined) {
				...
				if (systemResourceId) {
					this.nativeViewProtected.setNavigationIcon(
						systemResourceId
					);
				}
			} else if (navButton.icon) {
				const drawableOrId = loadActionIconDrawableOrResourceId(
					navButton
				);
				if (drawableOrId) {
	...
	public _updateIcon() {
		...
		if (visibility) {
			...
			if (icon !== undefined) {
				...
				let defaultIcon = application.android.nativeApp.getApplicationInfo()
					.icon;
	...
	public _addActionItems() {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (item.actionView && item.actionView.android) {
				...
				// Try to look in the system resources.
				let systemResourceId = getSystemResourceId(
					item.android.systemIcon
				);
				if (systemResourceId) {
