	...
	set titleView(value: View) {
		if (this._titleView !== value) {
			if (this._titleView) {
				...
				this._titleView.style[
					horizontalAlignmentProperty.cssName
				] = unsetValue;
				this._titleView.style[
					verticalAlignmentProperty.cssName
				] = unsetValue;
			}
...
export class ActionItemBase extends ViewBase implements ActionItemDefinition {
	...
	public onLoaded() {
		if (this._actionView) {
			this._actionView.style[horizontalAlignmentProperty.cssName] =
				"center";
			this._actionView.style[verticalAlignmentProperty.cssName] =
				"middle";
		}
		super.onLoaded();
	}
...
function onTitlePropertyChanged(
	actionBar: ActionBarBase,
	oldValue: string,
	newValue: string
) {
...
function convertToContentInset(
	this: void,
	value: string | Length
	...
	if (typeof value === "string" && value !== "auto") {
		...
		return [
			...
			[
				...
				Length.parse(insets[1] || insets[0]),
			],
		];
	} else {
		return [
...
>({ name: "iosIconRenderingMode", defaultValue: "alwaysOriginal" });
...
>({
	...
	getter: function (this: Style) {
		...
		return `${Length.convertToString(
			this.androidContentInsetLeft
		)} ${Length.convertToString(this.androidContentInsetRight)}`;
	},
