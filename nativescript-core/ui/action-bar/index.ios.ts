...
class TapBarItemHandlerImpl extends NSObject {
	...
	public static initWithOwner(
		owner: WeakRef<ActionItemDefinition>
...
export class ActionBar extends ActionBarBase {
	...
	public update() {
		...
		const navController = <UINavigationController>(
		...
		const indexOfViewController = navController.viewControllers.indexOfObject(
			viewController
		);
		if (
		...
		if (previousController) {
			if (this.navigationButton) {
				let tapHandler = TapBarItemHandlerImpl.initWithOwner(
		...
		if (img) {
			let image = img.imageWithRenderingMode(
	...
	private populateMenuItems(navigationItem: UINavigationItem) {
		...
		navigationItem.setRightBarButtonItemsAnimated(
			<any>rightBarItems,
			false
		);
	...
	private createBarButtonItem(item: ActionItemDefinition): UIBarButtonItem {
		const tapHandler = TapBarItemHandlerImpl.initWithOwner(
			new WeakRef(item)
		);
		...
		if (item.actionView && item.actionView.ios) {
			let recognizer = UITapGestureRecognizer.alloc().initWithTargetAction(
				tapHandler,
				"tap"
			);
			...
			if (img) {
				const image = img.imageWithRenderingMode(
	...
	public _onTitlePropertyChanged() {
		...
		let navigationItem: UINavigationItem = (<UIViewController>page.ios)
			.navigationItem;
		navigationItem.title = this.title;
	}
	...
	private get navBar(): UINavigationBar {
		...
		return (<UINavigationController>page.frame.ios.controller)
			.navigationBar;
	}
