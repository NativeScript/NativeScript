	...
	public viewWillTransitionToSizeWithTransitionCoordinator(
		...
		coordinator.animateAlongsideTransitionCompletion(null, () => {
			...
			if (owner && owner.items) {
				owner.items.forEach((tabItem) =>
					tabItem._updateTitleAndIconPositions()
				);
			}
		});
	}
...
class UITabBarControllerDelegateImpl extends NSObject
	...
	public static initWithOwner(
		...
		let delegate = <UITabBarControllerDelegateImpl>(
			UITabBarControllerDelegateImpl.new()
		);
	...
	public tabBarControllerShouldSelectViewController(
	...
	public tabBarControllerDidSelectViewController(
		tabBarController: UITabBarController,
		viewController: UIViewController
	): void {
		if (Trace.isEnabled()) {
...
class UINavigationControllerDelegateImpl extends NSObject
	...
	public static initWithOwner(
		...
		let delegate = <UINavigationControllerDelegateImpl>(
			UINavigationControllerDelegateImpl.new()
		);
		delegate._owner = owner;
	...
	navigationControllerWillShowViewControllerAnimated(
		...
		if (owner) {
			...
			let backToMoreWillBeVisible = owner._ios.viewControllers.containsObject(
				viewController
			);
			owner._handleTwoNavigationBars(backToMoreWillBeVisible);
		}
...
export class TabViewItem extends TabViewItemBase {
	...
	public _update() {
		...
		if (parent && controller) {
			...
			const title = getTransformedText(
				this.title,
...
export class TabView extends TabViewBase {
	...
	constructor() {
		...
		this.viewController = this._ios = UITabBarControllerImpl.initWithOwner(
			new WeakRef(this)
		);
	...
	public _handleTwoNavigationBars(backToMoreWillBeVisible: boolean) {
		if (Trace.isEnabled()) {
			Trace.write(
				`TabView._handleTwoNavigationBars(backToMoreWillBeVisible: ${backToMoreWillBeVisible})`,
				Trace.categories.Debug
			);
		}
		...
		const page =
			this.page ||
			this._selectedView.page ||
			(<any>this)._selectedView.currentPage;
		if (!page || !page.frame) {
			return;
		}
	...
	private getViewController(item: TabViewItem): UIViewController {
		let newController: UIViewController = item.view
			? item.view.viewController
			: null;
		...
		if (item.view.ios instanceof UIViewController) {
			...
			item.setViewController(
				newController,
				item.view.nativeViewProtected
			);
		}
	...
	public _getIcon(iconSource: string): UIImage {
		...
		if (!image) {
			...
			if (is && is.ios) {
				const originalRenderedImage = is.ios.imageWithRenderingMode(
	...
	[selectedIndexProperty.setNative](value: number) {
		if (Trace.isEnabled()) {
			Trace.write(
				"TabView._onSelectedIndexPropertyChangedSetNativeValue(" +
					value +
					")",
				Trace.categories.Debug
			);
		...
		if (value > -1) {
			(<any>(
				this._ios
			))._willSelectViewController = this._ios.viewControllers[value];
	...
	[tabTextFontSizeProperty.setNative](
		value: number | { nativeSize: number }
	) {
	...
	[iosIconRenderingModeProperty.setNative](
		value: "automatic" | "alwaysOriginal" | "alwaysTemplate"
	) {
		this._iconsCache = {};
...
function getTitleAttributesForStates(tabView: TabView): TabStates {
	...
	const tabItemFontSize =
		tabView.style.tabTextFontSize || defaultTabItemFontSize;
	const font: UIFont = (tabView.style.fontInternal || Font.default).getUIFont(
	...
	const selectedTextColor =
		tabSelectedItemTextColor instanceof Color
