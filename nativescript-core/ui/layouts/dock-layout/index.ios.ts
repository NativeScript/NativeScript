...
export class DockLayout extends DockLayoutBase {
	public onDockChanged(
		view: View,
		oldValue: "left" | "top" | "right" | "bottom",
		newValue: "left" | "top" | "right" | "bottom"
	) {
		this.requestLayout();
	}
	...
	public onMeasure(
		...
		this.eachLayoutChild((child, last) => {
			if (this.stretchLastChild && last) {
				childWidthMeasureSpec = layout.makeMeasureSpec(
					remainingWidth,
					widthMode
				);
				childHeightMeasureSpec = layout.makeMeasureSpec(
					remainingHeight,
					heightMode
				);
			} else {
				...
				childWidthMeasureSpec = layout.makeMeasureSpec(
					remainingWidth,
					widthMode === layout.EXACTLY ? layout.AT_MOST : widthMode
				);
				childHeightMeasureSpec = layout.makeMeasureSpec(
					remainingHeight,
					heightMode === layout.EXACTLY ? layout.AT_MOST : heightMode
				);
			}
