...
export class StackLayout extends StackLayoutBase {
		...
		this.eachLayoutChild((child, last) => {
			if (isVertical) {
				...
				childSize = View.measureChild(
					...
					child,
					childMeasureSpec,
					layout.makeMeasureSpec(remainingLength, measureSpec)
				);

				if (
					measureSpec === layout.AT_MOST &&
					this.isUnsizedScrollableView(child)
				) {
					Trace.write(
	...
	private layoutVertical(
		...
		switch (this.verticalAlignment) {
			case VerticalAlignment.MIDDLE:
				childTop =
					(bottom - top - this._totalLength) / 2 +
					paddingTop -
					paddingBottom;
				break;
	...
	private layoutHorizontal(
		...
		switch (this.horizontalAlignment) {
			case HorizontalAlignment.CENTER:
				childLeft =
					(right - left - this._totalLength) / 2 +
					paddingLeft -
					paddingRight;
				break;
	...
	private isUnsizedScrollableView(child: View): boolean {
		if (
			child.height === "auto" &&
			(child.ios instanceof UITableView ||
				child.ios instanceof UIScrollView)
		) {
			return true;
		}
