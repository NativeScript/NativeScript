...
function makeNativeSetter<T>(
	setter: (lp: org.nativescript.widgets.CommonLayoutParams, value: T) => void
) {
...
View.prototype[rowProperty.setNative] = makeNativeSetter<number>(
	(lp, value) => (lp.row = value)
);
View.prototype[columnProperty.setNative] = makeNativeSetter<number>(
	(lp, value) => (lp.column = value)
);
View.prototype[rowSpanProperty.setNative] = makeNativeSetter<number>(
	(lp, value) => (lp.rowSpan = value)
);
View.prototype[columnSpanProperty.setNative] = makeNativeSetter<number>(
	(lp, value) => (lp.columnSpan = value)
);

function createNativeSpec(
	itemSpec: ItemSpec
): org.nativescript.widgets.ItemSpec {
	switch (itemSpec.gridUnitType) {
		...
		case GridUnitType.PIXEL:
			return new org.nativescript.widgets.ItemSpec(
				itemSpec.value * layout.getDisplayDensity(),
