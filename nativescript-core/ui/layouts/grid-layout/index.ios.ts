...
export class GridLayout extends GridLayoutBase {
	...
	private getColumnIndex(view: View): number {
		return Math.max(
			0,
			Math.min(
				GridLayout.getColumn(view),
				this.columnsInternal.length - 1
			)
		);
	}
	...
	private getRowSpan(view: View, rowIndex: number): number {
		return Math.max(
			1,
			Math.min(
				GridLayout.getRowSpan(view),
				this.rowsInternal.length - rowIndex
			)
		);
	}
	...
	private getRowSpec(view: View): ItemSpec {
		return (
			this.rowsInternal[this.getRowIndex(view)] || this.helper.singleRow
		);
	}
	...
	public onMeasure(
		...
		const widthSizeAndState = View.resolveSizeAndState(
			measureWidth,
			width,
			widthMode,
			0
		);
		const heightSizeAndState = View.resolveSizeAndState(
			measureHeight,
			height,
			heightMode,
			0
	...
	public onLayout(
		...
		for (let i = 0, size = this.helper.columns.length; i < size; i++) {
			...
			columnGroup.rowOrColumn._actualLength = layout.round(
				layout.toDeviceIndependentPixels(roundedLength)
			);
		...
		for (let i = 0, size = this.helper.rows.length; i < size; i++) {
			...
			actualLength = offset - roundedOffset;
			roundedLength = Math.round(actualLength);
			rowGroup.rowOrColumn._actualLength = layout.round(
				layout.toDeviceIndependentPixels(roundedLength)
			);
			roundedOffset += roundedLength;
		...
		for (
			let i = 0, columns = this.helper.columns.length;
			i < columns;
			i++
		) {
			let columnGroup = this.helper.columns[i];
			for (
				let j = 0, children = columnGroup.children.length;
				j < children;
				j++
			) {
				let measureSpec = columnGroup.children[j];
				let childLeft = this.columnOffsets[
					measureSpec.getColumnIndex()
				];
				let childRight = this.columnOffsets[
					measureSpec.getColumnIndex() + measureSpec.getColumnSpan()
				];
				let childTop = this.rowOffsets[measureSpec.getRowIndex()];
				let childBottom = this.rowOffsets[
...
class MeasureHelper {
	...
	public addMeasureSpec(measureSpec: MeasureSpecs): void {
		...
		for (let i = measureSpec.getColumnIndex(); i < size; i++) {
			...
			if (columnGroup.getIsAuto()) {
				...
				measureSpec.pixelWidth += layout.toDevicePixels(
		...
		for (let i = measureSpec.getRowIndex(); i < size; i++) {
			...
			if (rowGroup.getIsAuto()) {
				...
				measureSpec.pixelHeight += layout.toDevicePixels(
	...
	private itemMeasured(
		measureSpec: MeasureSpecs,
		isFakeMeasure: boolean
	): void {
		if (!isFakeMeasure) {
	...
	private static updateStarLength(
		list: Array<ItemGroup>,
		starValue: number
	): void {
		let offset = 0;
		let roundedOffset = 0;
	...
	private fakeMeasure(): void {
		...
		for (let i = 0, size = this.columns.length; i < size; i++) {
			...
			for (
				let j = 0, childrenCount = columnGroup.children.length;
				j < childrenCount;
				j++
			) {
	...
	public measure(): void {
		...
		for (let i = 0; i < size; i++) {
			let columnGroup: ItemGroup = this.columns[i];
			for (
				let j = 0, childCount = columnGroup.children.length;
				j < childCount;
				j++
			) {
				let measureSpec: MeasureSpecs = columnGroup.children[j];
		...
		if (
			...
			MeasureHelper.updateStarLength(
				this.columns,
	...
	private measureChild(
		...
		let childSize = View.measureChild(
			this.grid,
			measureSpec.child,
			widthMeasureSpec,
			heightMeasureSpec
		...
		let columnSpanEnd: number =
			measureSpec.getColumnIndex() + measureSpec.getColumnSpan();
		let rowSpanEnd: number =
			measureSpec.getRowIndex() + measureSpec.getRowSpan();
		...
		if (measureSpec.autoColumnsCount > 0) {
			...
			if (remainingSpace > 0) {
				...
				for (
					let i = measureSpec.getColumnIndex();
					i < columnSpanEnd;
					i++
				) {
					let columnGroup: ItemGroup = this.columns[i];
	...
	private measureChildFixedColumns(measureSpec: MeasureSpecs): void {
		...
		let widthMeasureSpec = layout.makeMeasureSpec(
			measureWidth,
			this.stretchedHorizontally ? layout.EXACTLY : layout.AT_MOST
		);
		let heightMeasureSpec =
			measureSpec.autoRowsCount > 0
				? this.infinity
	...
	private measureChildFixedRows(measureSpec: MeasureSpecs): void {
		...
		let heightMeasureSpec = layout.makeMeasureSpec(
			measureHeight,
			this.stretchedVertically ? layout.EXACTLY : layout.AT_MOST
		);
	...
	private updateMinRowStarValueIfNeeded(
		measureSpec: MeasureSpecs,
		childMeasuredHeight: number
	): void {
	...
	private updateMinColumnStarValueIfNeeded(
		measureSpec: MeasureSpecs,
		childMeasuredWidth: number
	): void {
