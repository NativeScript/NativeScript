...
export class FlexboxLayout extends FlexboxLayoutBase {
	...
	private _measureHorizontal(
		...
		(() => {
			...
			for (let i = 0; i < childCount; i++) {
				...
				if (
					FlexBasisPercent.DEFAULT /*lp.flexBasisPercent*/ !==
						FlexBasisPercent.DEFAULT &&
					widthMode === EXACTLY
				) {
				...
				let childWidthMeasureSpec = FlexboxLayout.getChildMeasureSpec(
					widthMeasureSpec,
				...
				let childHeightMeasureSpec = FlexboxLayout.getChildMeasureSpec(
					heightMeasureSpec,
		...
		if (this.alignItems === AlignItems.BASELINE) {
			...
			this._flexLines.forEach((flexLine) => {
				...
				for (
					let i = viewIndex;
					i < viewIndex + flexLine._itemCount;
					i++
				) {
					let child = this._getReorderedChildAt(i);
	...
	private _determineMainSize(
		...
		this._flexLines.forEach((flexLine) => {
			if (flexLine.mainSize < mainSize) {
				childIndex = this._expandFlexItems(
					flexLine,
					flexDirection,
					mainSize,
					paddingAlongMainAxis,
				...
				childIndex = this._shrinkFlexItems(
					flexLine,
					flexDirection,
					mainSize,
					paddingAlongMainAxis,
			...
			if (this._isMainAxisDirectionHorizontal(flexDirection)) {
				if (!this._childrenFrozen[childIndex]) {
					...
					if (roundedCalculatedWidth > MAX_SIZE /* lp.maxWidth */) {
						...
						accumulatedRoundError =
							rawCalculatedWidth - roundedCalculatedWidth;
					}
					child.measure(
				...
				if (!this._childrenFrozen[childIndex]) {
					let flexGrow = FlexboxLayout.getFlexGrow(child);
					let rawCalculatedHeight =
						child.getMeasuredHeight() +
						unitSpace * flexGrow +
						accumulatedRoundError;
					let roundedCalculatedHeight = Math.round(
						rawCalculatedHeight
					);
					// TODO: MAX_SIZE is so big this is always false:
	...
	private _shrinkFlexItems(
		...
		if (
			flexLine._totalFlexShrink <= 0 ||
			maxMainSize > flexLine._mainSize
		) {
		...
		for (let i = 0; i < flexLine.itemCount; i++) {
			...
			if (this._isMainAxisDirectionHorizontal(flexDirection)) {
				...
				if (!this._childrenFrozen[childIndex]) {
					...
					const childWidthMeasureSpec = makeMeasureSpec(
					...
					const childHeightMeasureSpec = FlexboxLayout.getChildMeasureSpec(
						this._currentHeightMeasureSpec,
						lp.effectivePaddingTop +
							lp.effectivePaddingBottom +
							lp.effectiveMarginTop +
							lp.effectiveMarginBottom,
						lp.effectiveHeight < 0
							? WRAP_CONTENT
							: lp.effectiveHeight
					);
	...
	private _determineCrossSize(
		...
		if (mode === EXACTLY) {
			let totalCrossSize =
				this._getSumOfCrossSize() + paddingAlongCrossAxis;
				...
				switch (this.alignContent) {
					case AlignContent.STRETCH:
						(() => {
							let freeSpaceUnit =
								(size - totalCrossSize) /
								this._flexLines.length;
							let accumulatedError = 0;
							for (
								let i = 0,
									flexLinesSize = this._flexLines.length;
								i < flexLinesSize;
								i++
							) {
								let flexLine = this._flexLines[i];
								let newCrossSizeAsFloat =
									flexLine._crossSize + freeSpaceUnit;
								if (i === this._flexLines.length - 1) {
									newCrossSizeAsFloat += accumulatedError;
									accumulatedError = 0;
								}
								let newCrossSize = Math.round(
									newCrossSizeAsFloat
								);
								accumulatedError +=
									newCrossSizeAsFloat - newCrossSize;
					...
					case AlignContent.SPACE_BETWEEN:
						(() => {
							...
							for (
								let i = 0,
									flexLineSize = this._flexLines.length;
								i < flexLineSize;
								i++
							) {
								let flexLine = this._flexLines[i];
					...
					case AlignContent.CENTER: {
						...
						for (
							let i = 0, flexLineSize = this._flexLines.length;
							i < flexLineSize;
							i++
						) {
							if (i === 0) {
	...
	private _stretchViewVertically(view: View, crossSize: number) {
		...
		view.measure(
			childWidthMeasureSpec,
			makeMeasureSpec(newHeight, EXACTLY)
		);

		if (originalMeasuredWidth > view.getMeasuredWidth()) {
			childWidthMeasureSpec = makeMeasureSpec(
				originalMeasuredWidth,
				EXACTLY
			);
			view.measure(
				childWidthMeasureSpec,
	...
	private _setMeasuredDimensionForFlex(
		...
		switch (widthMode) {
			case EXACTLY:
				...
				widthSizeAndState = View.resolveSizeAndState(
					widthSize,
					widthSize,
					widthMode,
					childState
			...
			case EXACTLY:
				if (heightSize < calculatedMaxHeight) {
					childState = View.combineMeasuredStates(
						childState,
						MEASURED_STATE_TOO_SMALL >>
							layout.MEASURED_HEIGHT_STATE_SHIFT
					);
				}
				heightSizeAndState = View.resolveSizeAndState(
					heightSize,
					heightSize,
					heightMode,
					childState
	...
	private _isWrapRequired(
		...
		childLength: number,
		childAbsoluteIndex: number,
		childRelativeIndexInFlexLine: number
	): boolean {
	...
	private _getLargestMainSize(): number {
		return this._flexLines.reduce(
			(max, flexLine) => Math.max(max, flexLine.mainSize),
			Number.MIN_VALUE
		);
	}
	...
	private _getSumOfCrossSize(): number {
		// Omit divider
		return this._flexLines.reduce(
			(sum, flexLine) => sum + flexLine._crossSize,
			0
		);
	}
	...
	public onLayout(left: number, top: number, right: number, bottom: number) {
		...
		switch (this.flexDirection) {
			...
			default:
				throw new Error(
					"Invalid flex direction is set: " + this.flexDirection
				);
		}
	}
	...
	private _layoutHorizontal(
		...
		this._flexLines.forEach((flexLine, i) => {
			...
			switch (this.justifyContent) {
				...
				default:
					throw new Error(
						"Invalid justifyContent is set: " + this.justifyContent
					);
			}
			...
			for (let j = 0; j < flexLine.itemCount; j++) {
				...
				if (this.flexWrap === FlexWrap.WRAP_REVERSE) {
					if (isRtl) {
						this._layoutSingleChildHorizontal(
							child,
							flexLine,
							this.flexWrap,
							this.alignItems,
	...
	private _layoutSingleChildHorizontal(
		...
		switch (alignItems) {
			...
			case AlignItems.CENTER:
				let topFromCrossAxis =
					(crossSize - view.getMeasuredHeight()) / 2;
				if (flexWrap !== FlexWrap.WRAP_REVERSE) {
					view.layout(
						left,
						top +
		...
		this._flexLines.forEach((flexLine) => {
			...
			switch (this.justifyContent) {
				...
				case JustifyContent.SPACE_AROUND:
					...
					childBottom =
						height - paddingBottom - spaceBetweenItem / 2.0;
					break;
				case JustifyContent.SPACE_BETWEEN:
					childTop = paddingTop;
					let denominator =
			...
			for (let j = 0; j < flexLine.itemCount; j++) {
				...
				if (isRtl) {
					if (fromBottomToTop) {
						this._layoutSingleChildVertical(
							child,
							flexLine,
							true,
							this.alignItems,
	...
	private _layoutSingleChildVertical(
		...
		switch (alignItems) {
			...
			case AlignItems.FLEX_END:
				...
				break;
			case AlignItems.CENTER:
				let leftFromCrossAxis =
					(crossSize - view.getMeasuredWidth()) / 2;
				if (!isRtl) {
					view.layout(
						left +
							leftFromCrossAxis +
