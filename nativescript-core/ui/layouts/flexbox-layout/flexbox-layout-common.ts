...
export namespace FlexDirection {
	...
	export const isValid = makeValidator<FlexDirection>(
		ROW,
		ROW_REVERSE,
		COLUMN,
		COLUMN_REVERSE
...
export namespace JustifyContent {
	...
	export const isValid = makeValidator<JustifyContent>(
		FLEX_START,
		FLEX_END,
		CENTER,
		SPACE_BETWEEN,
...
export namespace AlignItems {
	...
	export const isValid = makeValidator<AlignItems>(
		FLEX_START,
		FLEX_END,
		CENTER,
		BASELINE,
...
export namespace AlignContent {
	...
	export const isValid = makeValidator<AlignContent>(
		FLEX_START,
		FLEX_END,
		CENTER,
		SPACE_BETWEEN,
...
export namespace AlignSelf {
	...
	export const isValid = makeValidator<AlignSelf>(
		AUTO,
		FLEX_START,
		FLEX_END,
		CENTER,
	...
	converter: function (value: string) {
		...
		if (value === unsetValue) {
			...
			if (trimmed) {
				...
				if (values.length >= 2) {
					if (
						FlexGrow.isValid(values[0]) &&
						FlexShrink.isValid(values[1])
					) {
						properties.push([
							flexGrowProperty,
							FlexGrow.parse(values[0]),
