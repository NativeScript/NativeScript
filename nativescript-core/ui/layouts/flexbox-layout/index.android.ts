...
function makeNativeSetter<T>(
	setter: (
		lp: org.nativescript.widgets.FlexboxLayout.LayoutParams,
		value: T
	) => void
) {
...
View.prototype[orderProperty.setNative] = makeNativeSetter<Order>(
	(lp, value) => (lp.order = value)
);
View.prototype[flexGrowProperty.setNative] = makeNativeSetter<FlexGrow>(
	(lp, value) => (lp.flexGrow = value)
);
View.prototype[flexShrinkProperty.setNative] = makeNativeSetter<FlexShrink>(
	(lp, value) => (lp.flexShrink = value)
);
View.prototype[flexWrapBeforeProperty.setNative] = makeNativeSetter<
	FlexWrapBefore
>((lp, value) => (lp.wrapBefore = value));
View.prototype[alignSelfProperty.setNative] = makeNativeSetter<AlignSelf>(
	(lp, value) => (lp.alignSelf = alignSelfMap[value])
);
...
export class FlexboxLayout extends FlexboxLayoutBase {
	...
	[flexDirectionProperty.setNative](flexDirection: FlexDirection) {
		this.nativeViewProtected.setFlexDirection(
	...
	public _updateNativeLayoutParams(child: View): void {
		...
		const lp = <org.nativescript.widgets.FlexboxLayout.LayoutParams>(
			child.nativeViewProtected.getLayoutParams()
		);
