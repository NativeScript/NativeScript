...
function createAlertDialog(
	...
	alert.setMessage(
		options && isString(options.message) ? options.message : ""
	);
...
function showDialog(builder: android.app.AlertDialog.Builder) {
	...
	if (labelColor) {
		...
		if (messageTextViewId) {
			const messageTextView = <android.widget.TextView>(
				dlg.findViewById(messageTextViewId)
			);
			if (messageTextView) {
...
function addButtonsToAlertDialog(
	alert: android.app.AlertDialog.Builder,
...
export function prompt(arg: any): Promise<PromptResult> {
	...
	return new Promise<PromptResult>((resolve, reject) => {
		try {
			...
			const input = new android.widget.EditText(
				androidApp.foregroundActivity
			);
	...
	return new Promise<LoginResult>((resolve, reject) => {
		try {
			...
			userNameInput.setHint(
				options.userNameHint ? options.userNameHint : ""
			);
	...
	return new Promise<string>((resolve, reject) => {
		try {
			...
			const message =
				options && isString(options.message) ? options.message : "";
			const title =
				options && isString(options.title) ? options.title : "";
			if (options && options.cancelable === false) {
				alert.setCancelable(false);
			}
