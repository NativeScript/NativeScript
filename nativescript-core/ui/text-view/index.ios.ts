...
import {
	TextViewBase as TextViewBaseCommon,
	maxLinesProperty,
} from "./text-view-common";
import {
	editableProperty,
	hintProperty,
	textProperty,
...
class UITextViewDelegateImpl extends NSObject implements UITextViewDelegate {
	...
	public textViewShouldChangeTextInRangeReplacementText(
		textView: UITextView,
		range: NSRange,
		...
		const owner = this._owner.get();
		if (owner) {
			return owner.textViewShouldChangeTextInRangeReplacementText(
				textView,
				range,
				replacementString
			);
...
class NoScrollAnimationUITextView extends UITextView {
	...
	public setContentOffsetAnimated(
...
export class TextView extends TextViewBaseCommon {
	...
	initNativeView() {
		...
		this._delegate = UITextViewDelegateImpl.initWithOwner(
			new WeakRef(this)
		);
	}
	...
	[borderTopWidthProperty.setNative](value: Length) {
		...
		let top = layout.toDeviceIndependentPixels(
	...
	[borderRightWidthProperty.setNative](value: Length) {
		let inset = this.nativeTextViewProtected.textContainerInset;
		let right = layout.toDeviceIndependentPixels(
	...
	[borderBottomWidthProperty.setNative](value: Length) {
		let inset = this.nativeTextViewProtected.textContainerInset;
		let bottom = layout.toDeviceIndependentPixels(
	...
	[borderLeftWidthProperty.setNative](value: Length) {
		let inset = this.nativeTextViewProtected.textContainerInset;
		let left = layout.toDeviceIndependentPixels(
	...
	[maxLinesProperty.setNative](value: number) {
		...
		if (value !== 0) {
			this.nativeTextViewProtected.textContainer.lineBreakMode =
