...
export class ListPicker extends ListPickerBase {
	...
	initNativeView() {
		...
		this._delegate = ListPickerDelegateImpl.initWithOwner(
			new WeakRef(this)
		);
	}
	...
	public pickerViewAttributedTitleForRowForComponent(
		...
		if (owner) {
			let title = NSAttributedString.alloc().initWithStringAttributes(
				...
				<any>{ [NSForegroundColorAttributeName]: pickerView.tintColor }
			);
