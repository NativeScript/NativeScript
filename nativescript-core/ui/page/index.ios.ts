...
function isBackNavigationTo(page: Page, entry): boolean {
	...
	const isReplace =
		navigationContext.navigationType === NavigationType.replace;
	if (isReplace) {
		return false;
	}
...
function isBackNavigationFrom(
	controller: UIViewControllerImpl,
	page: Page
): boolean {
	if (!page.frame) {
		return false;
	...
	if (
		...
		controller.navigationController.viewControllers.containsObject(
			controller
		)
	) {
		return false;
...
class UIViewControllerImpl extends UIViewController {
	...
	public viewWillAppear(animated: boolean): void {
		...
		const frame = this.navigationController
			? (<any>this.navigationController).owner
			: null;
	...
	public viewDidAppear(animated: boolean): void {
		...
		const frame: Frame = navigationController
			? (<any>navigationController).owner
			: null;
		...
		if (!owner._presentedViewController && frame) {
			...
			if (isReplace) {
				...
				if (controller) {
					const animated = frame._getIsAnimatedNavigation(
						newEntry.entry
					);
					if (animated) {
	...
	public viewWillDisappear(animated: boolean): void {
		...
		if (
			...
			const willSelectViewController =
				tab && (<any>tab)._willSelectViewController;
			if (
	...
	public get preferredStatusBarStyle(): UIStatusBarStyle {
		const owner = this._owner.get();
		if (owner) {
			return owner.statusBarStyle === "dark"
...
export class Page extends PageBase {
	...
	constructor() {
		super();
		const controller = UIViewControllerImpl.initWithOwner(
			new WeakRef(this)
		);
		...
		if (majorVersion <= 10) {
			...
			insets.top = layout.round(
				layout.toDevicePixels(
					this.viewController.view.safeAreaLayoutGuide.layoutFrame
						.origin.y
				)
			);
		}
	...
	public _addViewToNativeVisualTree(child: View, atIndex: number): boolean {
		...
		const viewController =
			child.ios instanceof UIViewController
				? child.ios
				: child.viewController;
		if (viewController) {
			// Adding modal controllers to as child will make app freeze.
			if (
				this.viewController.presentedViewController === viewController
			) {
				return true;
			}
	...
	[statusBarStyleProperty.setNative](value: string | UIBarStyle) {
		const frame = this.frame;
		if (frame) {
			const navigationBar = (<UINavigationController>frame.ios.controller)
				.navigationBar;
...
function invalidateTopmostController(controller: UIViewController): void {
	...
	while (size > 0) {
		...
		if (childController instanceof UITabBarController) {
			...
			invalidateTopmostController(
				childController.viewControllers.lastObject
			);
		} else {
