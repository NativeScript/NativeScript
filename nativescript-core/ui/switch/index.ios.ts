...
export class Switch extends SwitchBase {
	...
	public initNativeView(): void {
		...
		this._handler = SwitchChangeHandlerImpl.initWithOwner(
			new WeakRef(this)
		);
	...
	public onMeasure(
		...
		const widthAndState = Switch.resolveSizeAndState(
			layout.toDevicePixels(nativeSize.width),
			layout.toDevicePixels(51),
			layout.EXACTLY,
			0
		);
		const heightAndState = Switch.resolveSizeAndState(
			layout.toDevicePixels(nativeSize.height),
			layout.toDevicePixels(31),
			layout.EXACTLY,
			0
		);
