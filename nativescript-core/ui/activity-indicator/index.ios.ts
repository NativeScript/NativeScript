import {
	...
	busyProperty,
} from "./activity-indicator-common";
...
export class ActivityIndicator extends ActivityIndicatorBase {
	...
	private _activityIndicatorViewStyle =
		majorVersion <= 12 || !UIActivityIndicatorViewStyle.Medium
			? UIActivityIndicatorViewStyle.Gray
			: UIActivityIndicatorViewStyle.Medium;

	createNativeView() {
		const viewStyle = this._activityIndicatorViewStyle;
		const view = UIActivityIndicatorView.alloc().initWithActivityIndicatorStyle(
			viewStyle
		);
