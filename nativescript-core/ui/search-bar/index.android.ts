...
import {
	...
	textProperty,
	hintProperty,
	textFieldHintColorProperty,
	textFieldBackgroundColorProperty,
...
function initializeNativeClasses(): void {
	...
	class CompatQueryTextListenerImpl extends java.lang.Object
		implements androidx.appcompat.widget.SearchView.OnQueryTextListener {
	...
	class CompatCloseListenerImpl extends java.lang.Object
		implements androidx.appcompat.widget.SearchView.OnCloseListener {
		constructor(private owner: SearchBar) {
			super();
...
export class SearchBar extends SearchBarBase {
	...
	public createNativeView() {
		const nativeView = new androidx.appcompat.widget.SearchView(
			this._context
		);
	...
	private _getTextView(): android.widget.TextView {
		if (!this._searchTextView) {
			const pkgName = this.nativeViewProtected
				.getContext()
				.getPackageName();
			const id = this.nativeViewProtected
				.getContext()
				.getResources()
				.getIdentifier("search_src_text", "id", pkgName);
			this._searchTextView = <android.widget.TextView>(
				this.nativeViewProtected.findViewById(id)
			);
		}
	...
	private _getSearchPlate(): android.widget.LinearLayout {
		if (!this._searchPlate) {
			...
			this._searchPlate = <android.widget.LinearLayout>(
				this.nativeViewProtected.findViewById(id)
			);
		}
