...
function initializeDateChangedListener(): void {
	...
	class DateChangedListenerImpl extends java.lang.Object
		...
		onDateChanged(
			picker: android.widget.DatePicker,
			year: number,
			month: number,
			day: number
		) {
...
export class DatePicker extends DatePickerBase {
	...
	private updateNativeDate(): void {
		const nativeView = this.nativeViewProtected;
		const year =
			typeof this.year === "number" ? this.year : nativeView.getYear();
		const month =
