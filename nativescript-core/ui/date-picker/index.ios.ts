...
export class DatePicker extends DatePickerBase {
	...
	[dateProperty.setNative](value: Date) {
		...
		const comps = NSCalendar.currentCalendar.componentsFromDate(
	...
	[maxDateProperty.setNative](value: Date) {
		const picker = this.nativeViewProtected;
		const nsDate = NSDate.dateWithTimeIntervalSince1970(
			value.getTime() / 1000
		);
...
class UIDatePickerChangeHandlerImpl extends NSObject {
	...
	public static initWithOwner(
		owner: WeakRef<DatePicker>
	): UIDatePickerChangeHandlerImpl {
		const impl = <UIDatePickerChangeHandlerImpl>(
			UIDatePickerChangeHandlerImpl.new()
		);
		impl._owner = owner;
	...
	public valueChanged(sender: UIDatePicker) {
		...
		if (dateChanged) {
			dateProperty.nativeValueChange(
				owner,
