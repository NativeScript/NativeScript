...
function getSystemFont(
	...
	if (italic) {
		let descriptor = result.fontDescriptor.fontDescriptorWithSymbolicTraits(
			symbolicTraits
		);
...
function createUIFont(font: Font, defaultFont: UIFont): UIFont {
	...
	for (let i = 0; i < fontFamilies.length; i++) {
		...
		if (shouldUseSystemFont(fontFamily)) {
			...
			let descriptor = UIFontDescriptor.fontDescriptorWithFontAttributes(
				<any>fontAttributes
			);
			result = UIFont.fontWithDescriptorSize(descriptor, size);
			...
			if (font.isItalic && !actualItalic && EMULATE_OBLIQUE) {
				...
				descriptor = descriptor.fontDescriptorWithMatrix(
					OBLIQUE_TRANSFORM
				);
				result = UIFont.fontWithDescriptorSize(descriptor, size);
			}
			...
			if (Trace.isEnabled()) {
				Trace.write(
					"Error occur while registering font: " +
						CFErrorCopyDescription(<NSError>error.value),
...
function registerFontsInFolder(fontsFolderPath) {
	...
	fontsFolder.eachEntity((fileEntity: fs.FileSystemEntity) => {
		...
		if (
			...
			((<fs.File>fileEntity).extension === ".ttf" ||
				(<fs.File>fileEntity).extension === ".otf")
		) {
