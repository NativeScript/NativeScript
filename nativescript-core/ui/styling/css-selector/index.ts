...
export abstract class SelectorCore {
	 ...
	 */
	public abstract accumulateChanges(
		node: Node,
		map: ChangeAccumulator
	): boolean;
...
export class PseudoClassSelector extends SimpleSelector {
	...
	public match(node: Node): boolean {
		return (
			...
			node.cssPseudoClasses.has(this.cssPseudoClass)
		);
	}
	public mayMatch(node: Node): boolean {
		return true;
...
export class SimpleSelectorSequence extends SimpleSelector {
	...
	constructor(public selectors: SimpleSelector[]) {
		super();
		this.specificity = selectors.reduce(
			(sum, sel) => sel.specificity + sum,
			0
		);
		this.head = this.selectors.reduce(
			(prev, curr) => (!prev || curr.rarity > prev.rarity ? curr : prev),
			null
		);
...
export class Selector extends SelectorCore {
	...
	constructor(public selectors: SimpleSelector[]) {
		...
		this.groups = groups.map(
			(g) =>
				new Selector.ChildGroup(
					g.map((sg) => new Selector.SiblingGroup(sg))
				)
		);
...
export namespace Selector {
	...
	export class ChildGroup {
		...
		public match(node: Node): Node {
			return this.selectors.every(
				(sel, i) =>
					(node = i === 0 ? node : node.parent) && sel.match(node)
			)
				? node
				: null;
		}
		...
		public mayMatch(node: Node): Node {
			return this.selectors.every(
				(sel, i) =>
					(node = i === 0 ? node : node.parent) && sel.mayMatch(node)
			)
				? node
				: null;
		}
	...
	export class SiblingGroup {
		...
		public match(node: Node): Node {
			return this.selectors.every(
				(sel, i) =>
					(node = i === 0 ? node : getNodeDirectSibling(node)) &&
					sel.match(node)
			)
				? node
				: null;
...
function createSimpleSelectorSequenceFromAst(
	ast: parser.SimpleSelectorSequence
): SimpleSelectorSequence | SimpleSelector {
	if (ast.length === 0) {
		return new InvalidSelector(
			new Error("Empty simple selector sequence.")
		);
...
function createSelectorFromAst(
	ast: parser.Selector
): SimpleSelector | SimpleSelectorSequence | Selector {
	if (ast.length === 0) {
		...
		for (let i = 0; i < ast.length; i++) {
			simpleSelectorSequence = createSimpleSelectorSequenceFromAst(
				<parser.SimpleSelectorSequence>ast[i][0]
			);
...
export class SelectorsMap<T extends Node> implements LookupSorter {
	...
	query(node: T): SelectorsMatch<T> {
		...
		const selectors = selectorClasses.reduce(
