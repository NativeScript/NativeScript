...
export module ad {
	...
	function isSetColorFilterOnlyWidget(
	...
	export function onBackgroundOrBorderPropertyChanged(view: View) {
		...
		if (
			...
			const backgroundColor = ((<any>drawable).backgroundColor =
			...
			if (cachedDrawable) {
				if (
					...
					defaultDrawable = cachedDrawable.newDrawable(
						nativeView.getResources()
					);
				} else if (
					cachedDrawable instanceof android.graphics.drawable.Drawable
				) {
...
function refreshBorderDrawable(
	this: void,
	view: View,
	...
	if (background) {
		...
		const backgroundSizeParsedCSSValues = createNativeCSSValueArray(
			background.size
		);
...
function createNativeCSSValueArray(
	css: string
): native.Array<org.nativescript.widgets.CSSValue> {
	if (!css) {
		return null;
	}

	const cssValues = parse(css);
	const nativeArray = Array.create(
...
export function initImageCache(
	...
	if (!imageFetcher) {
		imageFetcher = org.nativescript.widgets.image.Fetcher.getInstance(
			context
		);
	} else {
