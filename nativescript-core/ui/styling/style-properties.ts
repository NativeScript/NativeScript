...
function toDevicePixelsCommon(
	length: PercentLength,
	auto: number = Number.NaN,
	parentAvailableWidth: number = Number.NaN
): number {
	if (length == "auto") {
		// tslint:disable-line
		return auto;
	...
	export function parse(fromValue: string | Length): PercentLength {
		...
		if (typeof fromValue === "string") {
			...
			if (percentIndex !== -1) {
				...
				if (
					percentIndex !== stringValue.length - 1 ||
					percentIndex === 0
				) {
					value = Number.NaN;
				} else {
...
const paddingProperty = new ShorthandProperty<Style, string | Length>({
	...
	getter: function (this: Style) {
		...
		return `${Length.convertToString(
			this.paddingTop
		)} ${Length.convertToString(
			this.paddingRight
		)} ${Length.convertToString(
			this.paddingBottom
...
export namespace HorizontalAlignment {
	...
	export const isValid = makeValidator<HorizontalAlignment>(
		LEFT,
		CENTER,
		RIGHT,
		STRETCH
...
export namespace VerticalAlignment {
	...
	export const isValid = makeValidator<VerticalAlignment>(
		TOP,
		MIDDLE,
		BOTTOM,
		STRETCH,
	...
	export const parse = (value: string) =>
		value.toLowerCase() === "center" ? MIDDLE : parseStrict(value);
...
function parseThickness(value: string): Thickness {
	if (typeof value === "string") {
		...
		if (arr.length === 1) {
			...
			throw new Error(
				"Expected 1, 2, 3 or 4 parameters. Actual: " + value
			);
		}
...
function convertToMargins(
	this: void,
	value: string | PercentLength
): [CssProperty<any, any>, any][] {
...
>({
	...
	equalityComparer: (value1, value2) => {
		if (
			value1 instanceof LinearGradient &&
			value2 instanceof LinearGradient
		) {
			...
			if (parsed) {
				...
				value =
					typeof background.image === "object"
						? LinearGradient.parse(background.image)
						: value;
			}
		}
...
export namespace BackgroundRepeat {
	...
	export const isValid = makeValidator<BackgroundRepeat>(
		REPEAT,
		REPEAT_X,
		REPEAT_Y,
		NO_REPEAT
...
function convertToBackgrounds(
	this: void,
	value: string
): [CssProperty<any, any>, any][] {
	if (typeof value === "string") {
		const backgrounds = parser.parseBackground(value).value;
		const backgroundColor = backgrounds.color
			? new Color(backgrounds.color)
			: unsetValue;
		...
		const backgroundPosition = backgrounds.position
...
function parseBorderColor(
	...
	if (value.indexOf("rgb") === 0) {
		result.top = result.right = result.bottom = result.left = new Color(
			value
		);
...
const borderColorProperty = new ShorthandProperty<Style, string | Color>({
	...
	getter: function (this: Style) {
		if (
			Color.equals(this.borderTopColor, this.borderRightColor) &&
...
export const borderTopColorProperty = new CssProperty<Style, Color>({
	...
	valueChanged: (target, oldValue, newValue) => {
		const background = target.backgroundInternal.withBorderTopColor(
			newValue
		);
		target.backgroundInternal = background;
	},
...
export const borderRightColorProperty = new CssProperty<Style, Color>({
	...
	valueChanged: (target, oldValue, newValue) => {
		const background = target.backgroundInternal.withBorderRightColor(
			newValue
		);
		target.backgroundInternal = background;
	},
...
export const borderBottomColorProperty = new CssProperty<Style, Color>({
	...
	valueChanged: (target, oldValue, newValue) => {
		const background = target.backgroundInternal.withBorderBottomColor(
			newValue
		);
		target.backgroundInternal = background;
	},
...
export const borderLeftColorProperty = new CssProperty<Style, Color>({
	...
	valueChanged: (target, oldValue, newValue) => {
		const background = target.backgroundInternal.withBorderLeftColor(
			newValue
		);
		target.backgroundInternal = background;
	},
...
export const borderRightWidthProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-right-width should be Non-Negative Finite number. Value: ${value}`
			);
		}
		...
		const background = target.backgroundInternal.withBorderRightWidth(
			value
		);
		target.backgroundInternal = background;
	},
...
export const borderBottomWidthProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-bottom-width should be Non-Negative Finite number. Value: ${value}`
			);
		}
		...
		const background = target.backgroundInternal.withBorderBottomWidth(
			value
		);
		target.backgroundInternal = background;
	},
...
export const borderLeftWidthProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-left-width should be Non-Negative Finite number. Value: ${value}`
			);
		}
...
export const borderTopLeftRadiusProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-top-left-radius should be Non-Negative Finite number. Value: ${value}`
			);
		}
		const background = target.backgroundInternal.withBorderTopLeftRadius(
			value
		);
		target.backgroundInternal = background;
	},
...
export const borderTopRightRadiusProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-top-right-radius should be Non-Negative Finite number. Value: ${value}`
			);
		}
		const background = target.backgroundInternal.withBorderTopRightRadius(
			value
		);
		target.backgroundInternal = background;
	},
...
export const borderBottomRightRadiusProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-bottom-right-radius should be Non-Negative Finite number. Value: ${value}`
			);
		}
		const background = target.backgroundInternal.withBorderBottomRightRadius(
			value
		);
		target.backgroundInternal = background;
	},
...
export const borderBottomLeftRadiusProperty = new CssProperty<Style, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (!isNonNegativeFiniteNumber(value)) {
			throw new Error(
				`border-bottom-left-radius should be Non-Negative Finite number. Value: ${value}`
			);
		}
		const background = target.backgroundInternal.withBorderBottomLeftRadius(
			value
		);
		target.backgroundInternal = background;
	},
...
export const fontFamilyProperty = new InheritedCssProperty<Style, string>({
	...
	valueChanged: (target, oldValue, newValue) => {
		...
		if (currentFont.fontFamily !== newValue) {
			...
			target.fontInternal = Font.equals(Font.default, newFont)
				? unsetValue
				: newFont;
		}
	},
...
export namespace Visibility {
	...
	export const parse = (value: string) =>
		value.toLowerCase() === "collapsed" ? COLLAPSE : parseStrict(value);
