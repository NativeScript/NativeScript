...
class CSSSource {
	...
	public static resolveCSSPathFromURL(
		...
		const file = resolveFileNameFromUrl(
			url,
			app,
			File.exists,
			importSource
	...
	private createSelectorsFromImports(): RuleSet[] {
		...
		const selectors = cssFiles.map(
			(file) => (file && file.selectors) || []
		);
	...
	private setPropertyValues(matchingSelectors: SelectorCore[]): void {
		...
		matchingSelectors.forEach((selector) =>
			selector.ruleset.declarations.forEach(
				(declaration) =>
					(newPropertyValues[declaration.property] =
						declaration.value)
			)
		);
...
export class StyleScope {
	...
	private _applyKeyframesOnSelectors() {
		for (let i = this._mergedCssSelectors.length - 1; i >= 0; i--) {
			...
			let animations: kam.KeyframeAnimationInfo[] =
				ruleset[animationsSymbol];
...
export function resolveFileNameFromUrl(
	...
	if (!isAbsolutePath) {
		...
		if (importSource) {
			const importFile = resolveFilePathFromImport(
				importSource,
				fileName
			);
