...
function initializeImageLoadedListener() {
	...
	class ImageLoadedListenerImpl extends java.lang.Object
		implements org.nativescript.widgets.image.Worker.OnImageLoadedListener {
...
export class Image extends ImageBase {
	...
	public _createImageSourceFromSrc(value: string | ImageSource | ImageAsset) {
		...
		let decodeWidth = Math.min(
			Length.toDevicePixels(this.decodeWidth, 0),
			screen.widthPixels
		);
		let decodeHeight = Math.min(
			Length.toDevicePixels(this.decodeHeight, 0),
			screen.heightPixels
		);
