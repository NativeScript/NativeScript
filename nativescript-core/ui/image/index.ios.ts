...
export class Image extends ImageBase {
	...
	public onMeasure(
		...
		const nativeWidth = this.imageSource
			? layout.toDevicePixels(this.imageSource.width)
			: 0;
		const nativeHeight = this.imageSource
			? layout.toDevicePixels(this.imageSource.height)
			: 0;
		...
		if (
			...
			const scale = Image.computeScaleFactor(
				width,
				height,
				finiteWidth,
				finiteHeight,
		...
		const widthAndState = Image.resolveSizeAndState(
			measureWidth,
			width,
			widthMode,
			0
		);
		const heightAndState = Image.resolveSizeAndState(
			measureHeight,
			height,
			heightMode,
			0
	...
	[stretchProperty.setNative](
		...
		switch (value) {
			case "aspectFit":
				this.nativeViewProtected.contentMode =
