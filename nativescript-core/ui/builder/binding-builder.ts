...
function isNamedParam(value) {
	...
	if (equalSignIndex > -1) {
		const equalSignSurround = value.substr(
			equalSignIndex > 0 ? equalSignIndex - 1 : 0,
			3
		);
		if (
			equalSignSurround.search(equalSignComparisionOperatorsRegex) === -1
		) {
			return true;
		}
	}
...
function isExpression(expression: string): boolean {
	if (expression.search(expressionSymbolsRegex) > -1) {
		...
		if (parentsMatches) {
			const restOfExpression = expression.substr(
...
export function getBindingOptions(name: string, value: string): any {
	...
	if (!areNamedParams(params)) {
		if (params.length === 1) {
			...
			if (isExpression(trimmedValue)) {
				...
				namedParams.push(
					bindingConstants.expression + " = " + trimmedValue
				);
			} else {
				sourceProp = trimmedValue;
			}
			namedParams.push(
				bindingConstants.sourceProperty + " = " + sourceProp
			);
			...
			namedParams.push(
				bindingConstants.expression + " = " + params[1].trim()
			);
			const twoWay = params[2]
				? params[2].toLowerCase().trim() === "true"
				: true;
	...
	const bindingOptionsParameters = parseNamedProperties(
		namedParams,
