...
export class Builder {
	static createViewFromEntry(entry: ViewEntry): View {
		if (entry.create) {
			...
			if (!view) {
				throw new Error(
					"Failed to create View with entry.create() function."
				);
			}
			...
			let moduleExports = resolvedCodeModuleName
			...
			if (moduleExports && moduleExports.createPage) {
				...
				const resolvedCssModuleName = resolveModuleName(
					moduleName,
					"css"
				); //entry.moduleName + ".css";
				...
				const componentView =
					componentModule && componentModule.component;
	...
	static parseMultipleTemplates(
		...
		return parseInternal(dummyComponent, context).component[
			"itemTemplates"
		];
	}
}
...
function loadCustomComponent(
	...
	if (resolvedXmlModuleName) {
		...
		if (!resolvedCssModuleName) {
			resolvedCssModuleName = resolveModuleName(
				componentNamespace,
				"css"
			);
		}
...
namespace xml2ui {
	...
	export class XmlStringParser extends XmlProducerBase
	...
	export function SourceErrorFormat(uri): ErrorFormatter {
		return (e: Error, p: xml.Position) => {
			const source = p
				? new Source(uri, p.line, p.column)
				: new Source(uri, -1, -1);
	...
	export class PlatformFilter extends XmlProducerBase
		implements XmlProducer, XmlConsumer {
	...
	export class TemplateParser implements XmlStateConsumer {
		...
		public parse(args: xml.ParserEvent): XmlStateConsumer {
			...
			return this._state === TemplateParser.State.FINISHED
				? this.parent
				: this;
		}
		...
		private parseEndElement(prefix: string, elementName: string) {
			if (this._state === TemplateParser.State.EXPECTING_START) {
				...
				throw new Error(
					"No more closing elements expected for this template."
				);
			}
			...
			if (this._nestingLevel === 0) {
				...
				if (
					...
					this._templateProperty.name in
	...
	export class MultiTemplateParser implements XmlStateConsumer {
		...
		public parse(args: xml.ParserEvent): XmlStateConsumer {
			if (
				...
				let childParser = new TemplateParser(
					this,
			...
			if (args.eventType === xml.ParserEventType.EndElement) {
				let name = ComponentParser.getComplexPropertyName(
					args.elementName
				);
	...
	export class ComponentParser implements XmlStateConsumer {
		...
		private complexProperties = new Array<
			ComponentParser.ComplexProperty
		>();
		...
		public parse(args: xml.ParserEvent): XmlStateConsumer {
			...
			const complexProperty = this.complexProperties[
			...
			if (args.eventType === xml.ParserEventType.StartElement) {
				if (ComponentParser.isComplexProperty(args.elementName)) {
					const name = ComponentParser.getComplexPropertyName(
						args.elementName
					);
					...
					if (ComponentParser.isKnownTemplate(name, parent.exports)) {
						return new TemplateParser(this, {
							context:
								(parent
									? getExports(parent.component)
									: null) || this.context, // Passing 'context' won't work if you set "codeFile" on the page
							parent: parent,
							name: name,
					...
					if (
						ComponentParser.isKnownMultiTemplate(
							name,
							parent.exports
						)
					) {
					...
					if (componentModule) {
						...
						if (parent) {
							if (complexProperty) {
								...
								(<any>parent.component)._addChildFromBuilder
							) {
								(<any>parent.component)._addChildFromBuilder(
									args.elementName,
									componentModule.component
				...
				if (ComponentParser.isComplexProperty(args.elementName)) {
					if (complexProperty) {
						if (complexProperty.parser) {
							...
							parent &&
							(<any>parent.component)._addArrayFromBuilder
						) {
		...
		private static addToComplexProperty(
			parent: ComponentModule,
			complexProperty: ComponentParser.ComplexProperty,
			...
			if (
				ComponentParser.isKnownCollection(
					complexProperty.name,
					parent.exports
				)
			) {
