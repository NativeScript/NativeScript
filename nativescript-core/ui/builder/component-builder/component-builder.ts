...
const createComponentInstance = profile(
	"createComponentInstance",
	(
		elementName: string,
		namespace: string
	): { instance: View; instanceModule: Object } => {
		let instance: View;
...
const getComponentModuleExports = profile(
	...
	(instance: View, moduleExports: Object, attributes: Object): Object => {
		if (attributes) {
			const codeFileAttribute =
				attributes[CODE_FILE] || attributes[IMPORT];
			if (codeFileAttribute) {
				const resolvedCodeFileModule = resolveModuleName(
					sanitizeModuleName(codeFileAttribute),
					""
				);
				if (resolvedCodeFileModule) {
					moduleExports = global.loadModule(
					...
					throw new Error(
						`Code file with path "${codeFileAttribute}" cannot be found! Looking for webpack module with name "${resolvedCodeFileModule}"`
					);
				}
			}
		}
...
const applyComponentCss = profile(
	"applyComponentCss",
	(instance: View, moduleName: string, attributes: Object) => {
		let cssApplied = false;

		if (attributes && attributes[CSS_FILE]) {
			let resolvedCssModuleName = resolveModuleName(
				sanitizeModuleName(attributes[CSS_FILE]),
				"css"
			);
			...
			if (resolvedCssModuleName) {
				...
				throw new Error(
					`Css file with path "${attributes[CSS_FILE]}" cannot be found! Looking for webpack module with name "${resolvedCssModuleName}"`
				);
			}
		}
...
const applyComponentAttributes = profile(
...
export function getComponentModule(
	...
	const { instance, instanceModule } = createComponentInstance(
		elementName,
		namespace,
		null
	);
...
export function setPropertyValue(
	...
	if (isBinding(propertyValue) && instance.bind) {
		const bindOptions = getBindingOptions(
			propertyName,
