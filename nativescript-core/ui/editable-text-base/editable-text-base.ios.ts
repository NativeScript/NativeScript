...
export abstract class EditableTextBase extends EditableTextBaseCommon {
	...
	[autocapitalizationTypeProperty.getDefault]():
		...
		| "sentences"
		| "allcharacters" {
		let autocapitalizationType = this.nativeTextViewProtected
		...
		switch (autocapitalizationType) {
			...
			default:
				throw new Error(
					"Invalid autocapitalizationType value:" +
						autocapitalizationType
				);
		}
	}
	...
	[autocorrectProperty.getDefault](): boolean | number {
		let autocorrectionType = this.nativeTextViewProtected
			.autocorrectionType;
...
function splice(
	...
	return (
		value.slice(0, start) +
		newSubStr +
		value.slice(start + Math.abs(delCount))
	);
}
