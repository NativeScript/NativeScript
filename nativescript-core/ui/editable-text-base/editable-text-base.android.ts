	...
	if (!dismissKeyboardTimeoutId) {
		dismissKeyboardTimeoutId = setTimeout(() => {
			...
			const activity = (owner &&
				owner._context) as androidx.appcompat.app.AppCompatActivity;
...
function initializeEditTextListeners(): void {
	...
	class EditTextListenersImpl extends java.lang.Object
		implements
			android.text.TextWatcher,
			android.view.View.OnFocusChangeListener,
			android.widget.TextView.OnEditorActionListener {
...
export abstract class EditableTextBase extends EditableTextBaseCommon {
	...
	[autocapitalizationTypeProperty.setNative](value: string) {
		...
		switch (value) {
			...
			case "words":
				inputType =
					inputType | android.text.InputType.TYPE_TEXT_FLAG_CAP_WORDS; //8192 (0x00020000) 14th bit
				break;
			case "sentences":
				inputType =
					inputType |
					android.text.InputType.TYPE_TEXT_FLAG_CAP_SENTENCES; //16384(0x00040000) 15th bit
				break;
			case "allcharacters":
				inputType =
					inputType |
					android.text.InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS; //4096 (0x00010000) 13th bit
				break;
			default:
				let number = +value;
	...
	[placeholderColorProperty.setNative](
	...
	[maxLengthProperty.setNative](value: number) {
		if (value === Number.POSITIVE_INFINITY) {
			...
			const lengthFilter = new android.text.InputFilter.LengthFilter(
				value
			);
			...
			for (let i = 0; i < filters.length; i++) {
				...
				if (
					!(filter instanceof android.text.InputFilter.LengthFilter)
				) {
