...
function getAttachListener(): android.view.View.OnAttachStateChangeListener {
	if (!attachStateChangeListener) {
		...
		class AttachListener extends java.lang.Object
...
export class Frame extends FrameBase {
	...
	private disposeCurrentFragment(): void {
		...
		const fragment: androidx.fragment.app.Fragment = this._currentEntry
			.fragment;
	...
	public setCurrent(
		...
		if (currentEntryChanged) {
			...
			if (this._tearDownPending) {
				...
				if (context && !entry.recreated) {
					entry.fragment = this.createFragment(
						entry,
						entry.fragmentTag
					);
		...
		if (navigationType === NavigationType.replace) {
			...
			const navigationTransition = this._getNavigationTransition(
				entry.entry
			);
	...
	public disposeNativeView() {
		...
		const current = this._currentEntry;
		const executingEntry = this._executingContext
	...
	public _getNavBarVisible(page: Page): boolean {
		switch (this.actionBarVisibility) {
			...
			default:
				...
				if (
					this._android &&
					this._android.showActionBar !== undefined
				) {
	...
	public _saveFragmentsState(): void {
		...
		this.backStack.forEach((entry) => {
			const view: android.view.View =
				entry.resolvedPage.nativeViewProtected;
...
function cloneExpandedTransitionListener(expandedTransitionListener: any) {
	...
	return addNativeTransitionListener(
...
function restoreTransitionState(
	...
	if (snapshot.enterTransitionListener) {
		expandedEntry.enterTransitionListener =
			snapshot.enterTransitionListener;
	}
...
class AndroidFrame extends Observable implements AndroidFrameDefinition {
	...
	public get activity(): androidx.appcompat.app.AppCompatActivity {
		let activity: androidx.appcompat.app.AppCompatActivity = this.owner
			._context;
		if (activity) {
			return activity;
		}
...
class FragmentCallbacksImplementation implements AndroidFragmentCallbacks {
	...
	public onCreateView(
		...
		if (savedState) {
			(<android.view.View>page.nativeViewProtected).restoreHierarchyState(
				savedState
			);
	...
	public onDestroyView(
		...
		try {
			...
			if (hasRemovingParent) {
				...
				this.frame.nativeViewProtected.setBackgroundDrawable(
					bitmapDrawable
				);
...
class ActivityCallbacksImplementation implements AndroidActivityCallbacks {
	...
	public onCreate(
		...
		if (savedInstanceState) {
			const rootViewId = savedInstanceState.getInt(
				ROOT_VIEW_ID_EXTRA,
				-1
			);
		...
		if (intent && intent.getAction()) {
			application.android.notify(<
				application.AndroidActivityNewIntentEventData
			>{
				eventName:
	...
	public onBackPressed(activity: any, superFunc: Function): void {
		...
		if (
			!viewArgs.cancel &&
			(view.onBackPressed === Frame.prototype.onBackPressed ||
				!view.onBackPressed())
		) {
	...
	public onRequestPermissionsResult(
		...
		application.android.notify(<
			application.AndroidActivityRequestPermissionsEventData
		>{
	...
	private setActivityContent(
		...
		if (!rootView) {
			...
			pushToSystemCssClasses(
				`${CLASS_PREFIX}${application.android.orientation}`
			);
			pushToSystemCssClasses(
				`${CLASS_PREFIX}${application.android.systemAppearance}`
			);
...
const notifyLaunch = profile("notifyLaunch", function notifyLaunch(
	intent: android.content.Intent,
	savedInstanceState: android.os.Bundle
): View {
...
export function setFragmentCallbacks(
	fragment: androidx.fragment.app.Fragment
): void {
