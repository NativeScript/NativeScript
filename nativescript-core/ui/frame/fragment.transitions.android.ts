...
const defaultInterpolator = lazy(
...
interface ExpandedTransitionListener
	extends androidx.transition.Transition.TransitionListener {
	entry: ExpandedEntry;
	transition: androidx.transition.Transition;
}
...
export function _setAndroidFragmentTransitions(
	...
	const currentFragment: androidx.fragment.app.Fragment = currentEntry
	...
	if (entries && entries.size > 0) {
		throw new Error(
			"Calling navigation before previous navigation finish."
		);
	}
	...
	if (name === "none") {
		...
		setupNewFragmentFadeTransition(
		...
		if (currentFragmentNeedsDifferentAnimation) {
			setupCurrentFragmentExplodeTransition(
				navigationTransition,
				currentEntry
			);
		}
		...
		const flipTransition = new FlipTransition(
			direction,
...
function setupAllAnimation(entry: ExpandedEntry, transition: Transition): void {
	...
	const enterAnimator = <ExpandedAnimator>(
		transition.createAndroidAnimator(AndroidTransitionType.enter)
	);
	...
	const popExitAnimator = <ExpandedAnimator>(
		transition.createAndroidAnimator(AndroidTransitionType.popExit)
	);
...
function setupExitAndPopEnterAnimation(
	entry: ExpandedEntry,
	transition: Transition
): void {
	const listener = getAnimationListener();
	...
	const exitAnimator = <ExpandedAnimator>(
		transition.createAndroidAnimator(AndroidTransitionType.exit)
	);
	...
	const popEnterAnimator = <ExpandedAnimator>(
		transition.createAndroidAnimator(AndroidTransitionType.popEnter)
	);
...
function getAnimationListener(): android.animation.Animator.AnimatorListener {
	if (!AnimationListener) {
		...
		class AnimationListenerImpl extends java.lang.Object
			...
			onAnimationRepeat(animator: ExpandedAnimator): void {
				if (Trace.isEnabled()) {
					Trace.write(
						`REPEAT ${animator.transitionType} for ${animator.entry.fragmentTag}`,
						Trace.categories.Transition
					);
				}
			}
			...
			onAnimationEnd(animator: ExpandedAnimator): void {
				if (Trace.isEnabled()) {
					Trace.write(
						`END ${animator.transitionType} for ${animator.entry.fragmentTag}`,
						Trace.categories.Transition
					);
				}
			...
			onAnimationCancel(animator: ExpandedAnimator): void {
				if (Trace.isEnabled()) {
					Trace.write(
						`CANCEL ${animator.transitionType} for ${animator.entry.fragmentTag}`,
						Trace.categories.Transition
					);
				}
			}
...
function clearAnimationListener(
	animator: ExpandedAnimator,
	...
	if (animator.entry && Trace.isEnabled()) {
		const entry = animator.entry;
		Trace.write(
			`Clear ${animator.transitionType} - ${entry.transition} for ${entry.fragmentTag}`,
			Trace.categories.Transition
		);
	}
...
export function _reverseTransitions(
	...
	if (reenterTransitionListener) {
		transitionUsed = true;
		previousFragment.setEnterTransition(
			reenterTransitionListener.transition
		);
	} else {
...
function getTransitionListener(
	entry: ExpandedEntry,
	transition: androidx.transition.Transition
): ExpandedTransitionListener {
	if (!TransitionListener) {
		@Interfaces([(<any>androidx).transition.Transition.TransitionListener])
		class TransitionListenerImpl extends java.lang.Object
			implements androidx.transition.Transition.TransitionListener {
			constructor(
			...
			public onTransitionStart(
				...
				if (Trace.isEnabled()) {
					Trace.write(
						`START ${toShortString(transition)} transition for ${
							entry.fragmentTag
						}`,
						Trace.categories.Transition
					);
			...
			onTransitionEnd(transition: androidx.transition.Transition): void {
				...
				if (Trace.isEnabled()) {
					Trace.write(
						`END ${toShortString(transition)} transition for ${
							entry.fragmentTag
						}`,
						Trace.categories.Transition
					);
			...
			onTransitionPause(
				...
				if (Trace.isEnabled()) {
					Trace.write(
						`PAUSE ${toShortString(transition)} transition for ${
			...
			onTransitionCancel(
				...
				if (Trace.isEnabled()) {
					Trace.write(
						`CANCEL ${toShortString(transition)} transition for ${
							this.entry.fragmentTag
						}`,
						Trace.categories.Transition
					);
...
function clearExitAndReenterTransitions(
	entry: ExpandedEntry,
	...
	if (exitListener) {
		...
		if (exitTransition) {
			...
			if (Trace.isEnabled()) {
				Trace.write(
					`Cleared Exit ${exitTransition
						.getClass()
						.getSimpleName()} transition for ${fragment}`,
					Trace.categories.Transition
				);
			}
		}
	...
	if (reenterListener) {
		...
		if (reenterTransition) {
			...
			if (Trace.isEnabled()) {
				Trace.write(
					`Cleared Reenter ${reenterTransition
						.getClass()
						.getSimpleName()} transition for ${fragment}`,
					Trace.categories.Transition
				);
...
function clearEntry(entry: ExpandedEntry, removeListener: boolean): void {
	...
	if (returnListener) {
		...
		if (returnTransition) {
			...
			if (Trace.isEnabled()) {
				Trace.write(
					`Cleared Return ${returnTransition
						.getClass()
						.getSimpleName()} transition for ${fragment}`,
					Trace.categories.Transition
				);
...
function allowTransitionOverlap(
...
function setupNewFragmentSlideTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry,
	name: string
): void {
...
function setupCurrentFragmentSlideTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry,
	name: string
): void {
...
function setupCurrentFragmentCustomTransition(
	...
	const exitAnimator = transition.createAndroidAnimator(
		AndroidTransitionType.exit
	);
	const exitTransition = new org.nativescript.widgets.CustomTransition(
		exitAnimator,
		transition.constructor.name + AndroidTransitionType.exit.toString()
	);

	setExitTransition(navTransition, entry, exitTransition);

	const reenterAnimator = transition.createAndroidAnimator(
		AndroidTransitionType.popEnter
	);
	const reenterTransition = new org.nativescript.widgets.CustomTransition(
		reenterAnimator,
		transition.constructor.name + AndroidTransitionType.popEnter.toString()
	);
...
function setupNewFragmentCustomTransition(
	...
	const enterAnimator = transition.createAndroidAnimator(
		AndroidTransitionType.enter
	);
	const enterTransition = new org.nativescript.widgets.CustomTransition(
		enterAnimator,
		transition.constructor.name + AndroidTransitionType.enter.toString()
	);
	setEnterTransition(navTransition, entry, enterTransition);

	const returnAnimator = transition.createAndroidAnimator(
		AndroidTransitionType.popExit
	);
	const returnTransition = new org.nativescript.widgets.CustomTransition(
		returnAnimator,
		transition.constructor.name + AndroidTransitionType.popExit.toString()
	);
	setReturnTransition(navTransition, entry, returnTransition);
}

function setupNewFragmentFadeTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry
): void {
	setupCurrentFragmentFadeTransition(navTransition, entry);

	const fadeInEnter = new androidx.transition.Fade(
		androidx.transition.Fade.IN
	);
	setEnterTransition(navTransition, entry, fadeInEnter);

	const fadeOutReturn = new androidx.transition.Fade(
		androidx.transition.Fade.OUT
	);
	setReturnTransition(navTransition, entry, fadeOutReturn);
}

function setupCurrentFragmentFadeTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry
): void {
	const fadeOutExit = new androidx.transition.Fade(
		androidx.transition.Fade.OUT
	);
	...
	const fadeInReenter = new androidx.transition.Fade(
		androidx.transition.Fade.IN
	);
	setReenterTransition(navTransition, entry, fadeInReenter);
}

function setupCurrentFragmentExplodeTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry
): void {
	...
	setReenterTransition(
		navTransition,
		entry,
		new androidx.transition.Explode()
	);
}

function setupNewFragmentExplodeTransition(
	navTransition: NavigationTransition,
	entry: ExpandedEntry
): void {
...
function setUpNativeTransition(
	navigationTransition: NavigationTransition,
	...
	const interpolator = navigationTransition.curve
		? _resolveAnimationCurve(navigationTransition.curve)
		: defaultInterpolator();
...
function transitionOrAnimationCompleted(entry: ExpandedEntry): void {
	...
	if (entries.size === 0) {
		...
		let current = frame.isCurrent(entry)
		...
		if (current) {
			setTimeout(() =>
				frame.setCurrent(current, navigationContext.navigationType)
			);
		}
	} else {
...
function toShortString(
	...
	return `${nativeTransition
		.getClass()
		.getSimpleName()}@${nativeTransition.hashCode().toString(16)}`;
}
...
function createDummyZeroDurationAnimator(
	...
	const animator = <android.animation.Animator>(
		android.animation.ObjectAnimator.ofFloat(null, "alpha", values)
	);
