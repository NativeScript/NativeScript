...
export class FrameBase extends CustomLayoutView implements FrameDefinition {
	...
	public canGoBack(): boolean {
		...
		this._navigationQueue.forEach((item) => {
			...
			const isBackNavigation =
				item.navigationType === NavigationType.back;
			if (isBackNavigation) {
				...
				previousForwardNotInBackstack =
					entry.entry.backstackVisible === false;
			}
		});
	...
	public _findEntryForTag(fragmentTag: string): BackstackEntry {
		...
		if (
			this._currentEntry &&
			this._currentEntry.fragmentTag === fragmentTag
		) {
			entry = this._currentEntry;
		} else {
			entry = this._backStack.find(
				(value) => value.fragmentTag === fragmentTag
			);
			// on API 26 fragments are recreated lazily after activity is destroyed.
			if (!entry) {
				const navigationItem = this._navigationQueue.find(
					(value) => value.entry.fragmentTag === fragmentTag
				);
	...
	public _goBackCore(backstackEntry: BackstackEntry) {
		if (Trace.isEnabled()) {
			Trace.write(
				`GO BACK CORE(${this._backstackEntryTrace(
					backstackEntry
	...
	public _navigateCore(backstackEntry: BackstackEntry) {
		if (Trace.isEnabled()) {
			Trace.write(
				`NAVIGATE CORE(${this._backstackEntryTrace(
					backstackEntry
				)}); currentPage: ${this.currentPage}`,
				Trace.categories.Navigation
			);
	...
	public _getNavigationTransition(
	...
	public _handleLivesync(context?: ModuleContext): boolean {
		...
		if (
			this.currentPage &&
			viewMatchesModuleContext(this.currentPage, context, [
				"markup",
				"script",
			])
		) {
	...
	protected replacePage(pagePath: string): void {
		...
		const newPage = <Page>(
			Builder.createViewFromEntry({ moduleName: contextModuleName })
		);
...
export const actionBarVisibilityProperty = new Property<
	FrameBase,
	"auto" | "never" | "always"
>({ name: "actionBarVisibility", defaultValue: "auto", affectsLayout: isIOS });
