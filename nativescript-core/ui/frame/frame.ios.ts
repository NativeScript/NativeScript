//Types
import {
	iOSFrame as iOSFrameDefinition,
	BackstackEntry,
...
export class Frame extends FrameBase {
	...
	public _animatedDelegate = <UINavigationControllerDelegate>(
	...
	public _navigateCore(backstackEntry: any) {
		...
		if (!this._currentEntry) {
			...
			this._ios.controller.pushViewControllerAnimated(
				viewController,
				animated
			);
			if (Trace.isEnabled()) {
		...
		if (clearHistory) {
			...
			for (let i = 0; i < oldControllers.count; i++) {
				(<any>(
					oldControllers.objectAtIndex(i)
				)).isBackstackCleared = true;
			}

			this._ios.controller.setViewControllersAnimated(
				newControllers,
				animated
			);
			if (Trace.isEnabled()) {
		...
		if (!Frame._isEntryBackstackVisible(this._currentEntry) || isReplace) {
			let newControllers = NSMutableArray.alloc<
				UIViewController
			>().initWithArray(this._ios.controller.viewControllers);
	...
	public _goBackCore(backstackEntry: BackstackEntry) {
		...
		let animated = this._currentEntry
...
class UINavigationControllerAnimatedDelegate extends NSObject
	...
	navigationControllerAnimationControllerForOperationFromViewControllerToViewController(
		...
		let animationController = _createIOSAnimatedTransitioning(
			navigationTransition,
			curve,
			operation,
			fromVC,
...
class UINavigationControllerImpl extends UINavigationController {
	...
	public static initWithOwner(
		...
		let controller = <UINavigationControllerImpl>(
			UINavigationControllerImpl.new()
		);
	...
	private animateWithDuration(
		...
		let duration = navigationTransition.duration
	...
	public popViewControllerAnimated(animated: boolean): UIViewController {
		...
		let nativeTransition = _getNativeTransition(
			navigationTransition,
			false
		);
		if (!animated || !navigationTransition || !nativeTransition) {
...
export function _getNativeCurve(
	...
	if (transition.curve) {
		switch (transition.curve) {
			...
			default:
				if (Trace.isEnabled()) {
					Trace.write(
						"Transition curve resolved to original: " +
							transition.curve,
						Trace.categories.Transition
					);
				}
