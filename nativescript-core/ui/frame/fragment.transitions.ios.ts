...
module UIViewControllerAnimatedTransitioningMethods {
	...
	const invocation = NSInvocation.invocationWithMethodSignature(
		methodSignature
	);
	invocation.selector = "completeTransition:";

	export function completeTransition(didComplete: boolean) {
		const didCompleteReference = new interop.Reference(
			interop.types.bool,
			didComplete
		);
...
class AnimatedTransitioning extends NSObject
	...
	public animateTransition(transitionContext: any): void {
		...
		const completion = UIViewControllerAnimatedTransitioningMethods.completeTransition.bind(
			transitionContext
		);
...
export function _createIOSAnimatedTransitioning(
	...
	nativeCurve: UIViewAnimationCurve,
	operation: UINavigationControllerOperation,
	fromVC: UIViewController,
	toVC: UIViewController
