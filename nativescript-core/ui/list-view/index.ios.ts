...
class ListViewCell extends UITableViewCell {
	...
	initWithStyleReuseIdentifier(
		...
		const cell = <this>(
			super.initWithStyleReuseIdentifier(style, reuseIdentifier)
		);
		// Clear background by default - this will make cells transparent
		cell.backgroundColor = null;
...
class DataSource extends NSObject implements UITableViewDataSource {
	...
	public tableViewNumberOfRowsInSection(
	...
	public tableViewCellForRowAtIndexPath(
		tableView: UITableView,
		indexPath: NSIndexPath
	): UITableViewCell {
...
class UITableViewDelegateImpl extends NSObject implements UITableViewDelegate {
	...
	public tableViewWillDisplayCellForRowAtIndexPath(
		tableView: UITableView,
...
class UITableViewRowHeightDelegateImpl extends NSObject
	...
	public static initWithOwner(
		...
		let delegate = <UITableViewRowHeightDelegateImpl>(
			UITableViewRowHeightDelegateImpl.new()
		);
		delegate._owner = owner;
...
export class ListView extends ListViewBase {
	...
	initNativeView() {
		...
		this._delegate = UITableViewDelegateImpl.initWithOwner(
			new WeakRef(this)
		);
	...
	private _scrollToIndex(index: number, animated: boolean = true) {
		...
		if (itemsLength > 0) {
			...
			Trace.write(
				`Cannot scroll listview to index ${index} when listview items not set`,
	...
	public isItemAtIndexVisible(itemIndex: number): boolean {
		const indexes: NSIndexPath[] = Array.from(
			this.ios.indexPathsForVisibleRows
		);
	...
	public _onRowHeightPropertyChanged(oldValue: Length, newValue: Length) {
		const value = layout.toDeviceIndependentPixels(
	...
	public measure(widthMeasureSpec: number, heightMeasureSpec: number): void {
		...
		const changed = this._setCurrentMeasureSpecs(
			widthMeasureSpec,
			heightMeasureSpec
		);
	...
	public onLayout(
		...
		this._map.forEach((childView, listViewCell) => {
			...
			let cellHeight =
				rowHeight > 0
					? rowHeight
					: this.getHeight(childView._listViewItemIndex);
			if (cellHeight) {
	...
	private _layoutCell(cellView: View, indexPath: NSIndexPath): number {
		if (cellView) {
			...
			const heightMeasureSpec: number =
				rowHeight >= 0
					? layout.makeMeasureSpec(rowHeight, layout.EXACTLY)
					: infinity;
			const measuredSize = View.measureChild(
				this,
				cellView,
