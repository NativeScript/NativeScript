...
@CSSType("ListView")
export abstract class ListViewBase extends ContainerView
	implements ListViewDefinition, TemplatedItemsView {
...
export const rowHeightProperty = new CoercibleProperty<ListViewBase, Length>({
	...
	valueChanged: (target, oldValue, newValue) => {
		target._effectiveRowHeight = Length.toDevicePixels(
			newValue,
