...
function initializeItemClickListener(): void {
	...
	class ItemClickListenerImpl extends java.lang.Object
		...
		onItemClick<T extends android.widget.Adapter>(
			parent: android.widget.AdapterView<T>,
			convertView: android.view.View,
			index: number,
			id: number
		) {
			const owner = this.owner;
			const view = owner._realizedTemplates
...
function ensureListViewAdapterClass() {
	...
	class ListViewAdapter extends android.widget.BaseAdapter {
		...
		public getItemViewType(index: number) {
			...
			let itemViewType = this.owner._itemTemplatesInternal.indexOf(
				template
			);
		...
		public getView(
			...
			let view: View;
			if (convertView) {
				view = this.owner._realizedTemplates
					.get(template.key)
					.get(convertView);
				if (!view) {
					throw new Error(
						`There is no entry with key '${convertView}' in the realized views cache for template with key'${template.key}'.`
					);
				}
			} else {
			...
			if (args.view) {
				...
				let realizedItemsForTemplateKey = this.owner._realizedTemplates.get(
					template.key
				);
