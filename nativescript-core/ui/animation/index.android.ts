...
let propertyKeys = {};
propertyKeys[Properties.backgroundColor] = Symbol(
	keyPrefix + Properties.backgroundColor
);
...
export function _resolveAnimationCurve(
	...
	switch (curve) {
		...
		case "ease":
			return (<any>(
				androidx
			)).core.view.animation.PathInterpolatorCompat.create(
				0.25,
				0.1,
				0.25,
				1.0
		...
		default:
			...
			if (curve instanceof CubicBezierAnimationCurve) {
				...
				<any>curve instanceof android.view.animation.LinearInterpolator
			) {
				return <android.view.animation.Interpolator>curve;
			} else {
...
function createAnimationSet(
	animators: android.animation.ObjectAnimator[],
	iterations: number
): android.animation.AnimatorSet {
	iterations = getAndroidRepeatCount(iterations);

	const animatorSet = new android.animation.AnimatorSet();
	const animatorsArray = Array.create(
...
export class Animation extends AnimationBase {
		...
		this._animatorListener = new android.animation.Animator.AnimatorListener(
			{
				onAnimationStart: function (
					...
					if (Trace.isEnabled()) {
						Trace.write(
							"MainAnimatorListener.onAndroidAnimationStart(" +
								animator +
								")",
							Trace.categories.Animation
						);
				...
				onAnimationRepeat: function (
					...
					if (Trace.isEnabled()) {
						Trace.write(
							"MainAnimatorListener.onAnimationRepeat(" +
								animator +
								")",
							Trace.categories.Animation
						);
				...
				onAnimationEnd: function (
					...
					if (Trace.isEnabled()) {
						Trace.write(
							"MainAnimatorListener.onAnimationEnd(" +
								animator +
								")",
							Trace.categories.Animation
						);
				...
				onAnimationCancel: function (
					...
					if (Trace.isEnabled()) {
						Trace.write(
							"MainAnimatorListener.onAnimationCancel(" +
								animator +
								")",
							Trace.categories.Animation
						);
	...
	public play(resetOnFinish?: boolean): AnimationPromise {
		...
		for (
			let i = 0, length = this._propertyAnimations.length;
			i < length;
			i++
		) {
	...
	private _createAnimators(propertyAnimation: PropertyAnimation): void {
		...
		if (
			...
			throw new Error(
				`Animation target cannot be null or undefined; property: ${propertyAnimation.property}; value: ${propertyAnimation.value};`
			);
		}
		...
		if (
			...
			throw new Error(
				`Animation property cannot be null or undefined; target: ${propertyAnimation.target}; value: ${propertyAnimation.value};`
			);
		}
		...
		if (
			...
			throw new Error(
				`Animation value cannot be null or undefined; target: ${propertyAnimation.target}; property: ${propertyAnimation.property};`
			);
		}

		this._target = propertyAnimation.target;

		const nativeView = <android.view.View>(
		...
		switch (propertyAnimation.property) {
			case Properties.opacity:
				...
				propertyResetCallbacks.push(
					checkAnimation(() => {
						...
						if (propertyAnimation.target.nativeViewProtected) {
							propertyAnimation.target[opacityProperty.setNative](
			...
			case Properties.backgroundColor:
				...
				nativeArray[0] = propertyAnimation.target.backgroundColor
					? java.lang.Integer.valueOf(
							(<Color>propertyAnimation.target.backgroundColor)
								.argb
					  )
					: java.lang.Integer.valueOf(-1);
				nativeArray[1] = java.lang.Integer.valueOf(
					(<Color>propertyAnimation.value).argb
				);
				let animator = android.animation.ValueAnimator.ofObject(
					argbEvaluator,
					nativeArray
				);
			...
			case Properties.translate:
				...
				propertyResetCallbacks.push(
					checkAnimation(() => {
						...
						if (propertyAnimation.target.nativeViewProtected) {
							propertyAnimation.target[
								translateXProperty.setNative
							](propertyAnimation.target.style.translateX);
							propertyAnimation.target[
								translateYProperty.setNative
					...
					createAnimationSet(
						[
							createObjectAnimator(
								nativeView,
								"translationX",
								propertyAnimation.value.x * density
							),
							createObjectAnimator(
								nativeView,
								"translationY",
								propertyAnimation.value.y * density
							),
						],
			...
			case Properties.scale:
				...
				propertyUpdateCallbacks.push(
					checkAnimation(() => {
						propertyAnimation.target.style[
							setLocal
								? scaleXProperty.name
								: scaleXProperty.keyframe
						] = propertyAnimation.value.x;
						propertyAnimation.target.style[
							setLocal
								? scaleYProperty.name
								: scaleYProperty.keyframe
						] = propertyAnimation.value.y;
					})
				);
				...
				propertyResetCallbacks.push(
					checkAnimation(() => {
						...
						if (propertyAnimation.target.nativeViewProtected) {
							propertyAnimation.target[scaleXProperty.setNative](
								propertyAnimation.target.style.scaleX
							);
							propertyAnimation.target[scaleYProperty.setNative](
				...
				propertyUpdateCallbacks.push(
					checkAnimation(() => {
						...
						propertyAnimation.target.style[
							setLocal
								? rotateProperty.name
								: rotateProperty.keyframe
						] = propertyAnimation.value.z;
					})
				);
				propertyResetCallbacks.push(
					checkAnimation(() => {
						...
						if (propertyAnimation.target.nativeViewProtected) {
							propertyAnimation.target[rotateProperty.setNative](
								propertyAnimation.target.style.rotate
							);
							propertyAnimation.target[rotateXProperty.setNative](
								propertyAnimation.target.style.rotateX
							);
							propertyAnimation.target[rotateYProperty.setNative](
								propertyAnimation.target.style.rotateY
							);
						}
					})
				);
			...
			case Properties.height: {
				...
				const extentProperty = isVertical
					? heightProperty
					: widthProperty;
				...
				const parentExtent: number = isVertical
				...
				let nativeHeight: number = isVertical
					? nativeView.getHeight()
					: nativeView.getWidth();
				const targetStyle: string = setLocal
					? extentProperty.name
					: extentProperty.keyframe;
				originalValue1 = nativeHeight / screen.mainScreen.scale;
				nativeArray[0] = originalValue1;
				nativeArray[1] = toValue;
				let extentAnimator = android.animation.ValueAnimator.ofFloat(
					nativeArray
				);
				...
				propertyUpdateCallbacks.push(
					checkAnimation(() => {
						propertyAnimation.target.style[targetStyle] =
							propertyAnimation.value;
					})
				);
				propertyResetCallbacks.push(
			...
			default:
				throw new Error(
					`Animating property '${propertyAnimation.property}' is unsupported`
				);
		}
		...
		for (let i = 0, length = animators.length; i < length; i++) {
			...
			if (
				...
				(<android.animation.ValueAnimator>animators[i]).setRepeatCount(
					getAndroidRepeatCount(propertyAnimation.iterations)
				);
			}
