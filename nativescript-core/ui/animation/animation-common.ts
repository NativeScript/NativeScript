...
export class CubicBezierAnimationCurve
	implements CubicBezierAnimationCurveDefinition {
	public x1: number;
	public y1: number;
	public x2: number;
	public y2: number;
...
export abstract class AnimationBase implements AnimationBaseDefinition {
	...
	constructor(
		animationDefinitions: Array<AnimationDefinition>,
	...
	protected _rejectAlreadyPlaying(): AnimationPromiseDefinition {
		...
		return <AnimationPromiseDefinition>(
	...
	public play(): AnimationPromiseDefinition {
		...
		const animationFinishedPromise = <AnimationPromiseDefinition>(
			new Promise<void>((resolve, reject) => {
	...
	private static _createPropertyAnimations(
		...
		for (let item in animationDefinition) {
			...
			if (
				...
				throw new Error(
					`Property ${item} must be valid number. Value: ${value}`
				);
			} else if (
				(item === Properties.scale || item === Properties.translate) &&
				(typeof (<Pair>value).x !== "number" ||
					typeof (<Pair>value).y !== "number")
			) {
				throw new Error(
					`Property ${item} must be valid Pair. Value: ${value}`
				);
			} else if (
				item === Properties.backgroundColor &&
				!Color.isValid(animationDefinition.backgroundColor)
			) {
				throw new Error(
					`Property ${item} must be valid color. Value: ${value}`
				);
			} else if (
				...
				if (
					...
					!(
						typeof rotate.x === "number" &&
						typeof rotate.y === "number" &&
						typeof rotate.z === "number"
					)
				) {
					throw new Error(
						`Property ${rotate} must be valid number or Point3D. Value: ${value}`
					);
				}
			}
		}
			...
			propertyAnimations.push({
				...
				value:
					typeof animationDefinition.backgroundColor === "string"
						? new Color(<any>animationDefinition.backgroundColor)
