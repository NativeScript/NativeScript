...
export class HtmlView extends HtmlViewBase {
	...
	private renderWithStyles() {
		...
		if (this.nativeViewProtected.font) {
			styles.push(
				`font-family: '${this.nativeViewProtected.font.fontName}';`
			);
			styles.push(
				`font-size: ${this.nativeViewProtected.font.pointSize}px;`
			);
		}
		if (this.nativeViewProtected.textColor) {
			const textColor = Color.fromIosColor(
				this.nativeViewProtected.textColor
			);
	...
	[linkColorProperty.getDefault](): UIColor {
		return this.nativeViewProtected.linkTextAttributes[
	...
	[fontInternalProperty.setNative](value: Font | UIFont) {
		const font =
			value instanceof Font
				? value.getUIFont(this.nativeViewProtected.font)
				: value;
