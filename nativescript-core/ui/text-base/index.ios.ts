...
class UILabelClickHandlerImpl extends NSObject {
	...
	public linkTap(tapGesture: UITapGestureRecognizer) {
		...
		if (owner) {
			...
			let textContainer = NSTextContainer.alloc().initWithSize(
				CGSizeZero
			);
			let textStorage = NSTextStorage.alloc().initWithAttributedString(
				owner.nativeTextViewProtected["attributedText"]
			);
			...
			let textBoundingBox = layoutManager.usedRectForTextContainer(
				textContainer
			);

			let textContainerOffset = CGPointMake(
				(labelSize.width - textBoundingBox.size.width) * 0.5 -
					textBoundingBox.origin.x,
				(labelSize.height - textBoundingBox.size.height) * 0.5 -
			...
			for (let i = 0; i < owner._spanRanges.length; i++) {
				...
				if (
					range.location <= indexOfCharacter &&
					range.location + range.length > indexOfCharacter
				) {
					if (
						owner.formattedText &&
						owner.formattedText.spans.length > i
					) {
...
export class TextBase extends TextBaseCommon {
	...
	public nativeTextViewProtected:
		| UITextField
		| UITextView
		| UILabel
		| UIButton;
	...
	_setTappableState(tappable: boolean) {
		if (this._tappable !== tappable) {
			...
			if (this._tappable) {
				const tapHandler = UILabelClickHandlerImpl.initWithOwner(
					new WeakRef(this)
				);
				// associate handler with menuItem or it will get collected by JSC.
				(<any>this).handler = tapHandler;

				this._tapGestureRecognizer = UITapGestureRecognizer.alloc().initWithTargetAction(
					tapHandler,
					"linkTap"
				);
				this.nativeViewProtected.userInteractionEnabled = true;
				this.nativeViewProtected.addGestureRecognizer(
					this._tapGestureRecognizer
				);
			} else {
				this.nativeViewProtected.userInteractionEnabled = false;
				this.nativeViewProtected.removeGestureRecognizer(
					this._tapGestureRecognizer
				);
			}
		}
	...
	[fontInternalProperty.setNative](value: Font | UIFont) {
		if (!(value instanceof Font) || !this.formattedText) {
			let nativeView = this.nativeTextViewProtected;
			nativeView =
				nativeView instanceof UIButton
					? nativeView.titleLabel
					: nativeView;
			const font =
				value instanceof Font
					? value.getUIFont(nativeView.font)
					: value;
	...
	[textAlignmentProperty.setNative](value: TextAlignment) {
		const nativeView = <UITextField | UITextView | UILabel>(
	...
	_setNativeText(reset: boolean = false): void {
		if (reset) {
			...
			if (nativeView instanceof UIButton) {
				...
				nativeView.setAttributedTitleForState(
					null,
	...
	_setColor(color: UIColor): void {
		if (this.nativeTextViewProtected instanceof UIButton) {
			this.nativeTextViewProtected.setTitleColorForState(
				color,
				UIControlState.Normal
			);
	...
	setFormattedTextDecorationAndTransform() {
		const attrText = this.createNSMutableAttributedString(
			this.formattedText
		);
		...
		if (this.letterSpacing !== 0) {
			attrText.addAttributeValueRange(
				NSKernAttributeName,
				this.letterSpacing *
					this.nativeTextViewProtected.font.pointSize,
		...
		if (this.style.lineHeight) {
			...
			if (this.nativeTextViewProtected instanceof UIButton) {
				paragraphStyle.alignment = (<UIButton>(
					this.nativeTextViewProtected
				)).titleLabel.textAlignment;
			} else {
				paragraphStyle.alignment = (<
					UITextField | UITextView | UILabel
				>this.nativeTextViewProtected).textAlignment;
			}
			...
			const paragraphStyle = NSMutableParagraphStyle.alloc().init();
			paragraphStyle.alignment = (<UITextView>(
				this.nativeTextViewProtected
			)).textAlignment;
			attrText.addAttributeValueRange(
				NSParagraphStyleAttributeName,
		...
		if (this.nativeTextViewProtected instanceof UIButton) {
			this.nativeTextViewProtected.setAttributedTitleForState(
				attrText,
				UIControlState.Normal
			);
		} else {
	...
	setTextDecorationAndTransform() {
		...
		if (style.letterSpacing !== 0 && this.nativeTextViewProtected.font) {
			...
			if (this.nativeTextViewProtected instanceof UITextField) {
				this.nativeTextViewProtected.defaultTextAttributes.setValueForKey(
					kern,
					NSKernAttributeName
				);
			}
		...
		const source = getTransformedText(
		...
		if (dict.size > 0 || isTextView) {
			if (isTextView && this.nativeTextViewProtected.font) {
				...
				dict.set(
					NSFontAttributeName,
					this.nativeTextViewProtected.font
				);
			}
			...
			if (this.nativeTextViewProtected instanceof UIButton) {
				...
				this.nativeTextViewProtected.setTitleForState(
					source,
					UIControlState.Normal
				);
			} else {
	...
	createNSMutableAttributedString(
		...
		if (formattedString && formattedString.parent) {
			for (
				let i = 0, spanStart = 0, length = formattedString.spans.length;
				i < length;
				i++
			) {
				const span = formattedString.spans.getItem(i);
				const text = span.text;
				const textTransform = (<TextBase>formattedString.parent)
					.textTransform;
				let spanText =
				...
				const nsAttributedString = this.createMutableStringForSpan(
					span,
					spanText
				);
				mas.insertAttributedStringAtIndex(
					nsAttributedString,
					spanStart
				);
	...
	createMutableStringForSpan(
		...
		const textDecoration: TextDecoration = getClosestPropertyValue(
			textDecorationProperty,
			span
		);
		...
		if (align) {
			attrDict[NSBaselineOffsetAttributeName] = this.getBaselineOffset(
				iosFont,
				align
			);
		}
...
function NSStringFromNSAttributedString(
	source: NSAttributedString | string
): NSString {
	return NSString.stringWithString(
		(source instanceof NSAttributedString && source.string) ||
			<string>source
	);
}
