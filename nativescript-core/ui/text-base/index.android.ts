...
function initializeTextTransformation(): void {
	...
	class TextTransformationImpl extends java.lang.Object
		...
		public getTransformation(charSeq: any, view: android.view.View): any {
			...
			if (formattedText) {
				return createSpannableStringBuilder(
					formattedText,
					(<android.widget.TextView>view).getTextSize()
				);
			} else {
...
export class TextBase extends TextBaseCommon {
	...
	[formattedTextProperty.setNative](value: FormattedString) {
		...
		if (!value) {
			if (
				...
				nativeView.getTransformationMethod() instanceof
					TextTransformation
			) {
				nativeView.setTransformationMethod(
	...
	[lineHeightProperty.getDefault](): number {
		return (
			this.nativeTextViewProtected.getLineHeight() /
			layout.getDisplayDensity()
		);
	}
	[lineHeightProperty.setNative](value: number) {
		const fontHeight = this.nativeTextViewProtected
			.getPaint()
			.getFontMetricsInt(null);
		this.nativeTextViewProtected.setLineSpacing(
			Math.max(value - fontHeight, 0) * layout.getDisplayDensity(),
			1
		);
	}
	...
	[paddingTopProperty.setNative](value: Length) {
		org.nativescript.widgets.ViewHelper.setPaddingTop(
			...
			Length.toDevicePixels(value, 0) +
				Length.toDevicePixels(this.style.borderTopWidth, 0)
		);
	}
	...
	[paddingRightProperty.setNative](value: Length) {
		org.nativescript.widgets.ViewHelper.setPaddingRight(
			this.nativeTextViewProtected,
			Length.toDevicePixels(value, 0) +
				Length.toDevicePixels(this.style.borderRightWidth, 0)
		);
	}
	...
	[paddingBottomProperty.setNative](value: Length) {
		org.nativescript.widgets.ViewHelper.setPaddingBottom(
			this.nativeTextViewProtected,
			Length.toDevicePixels(value, 0) +
				Length.toDevicePixels(this.style.borderBottomWidth, 0)
		);
	}
	...
	[paddingLeftProperty.setNative](value: Length) {
		org.nativescript.widgets.ViewHelper.setPaddingLeft(
			this.nativeTextViewProtected,
			Length.toDevicePixels(value, 0) +
				Length.toDevicePixels(this.style.borderLeftWidth, 0)
		);
	}
	...
	_setTappableState(tappable: boolean) {
		if (this._tappable !== tappable) {
			...
			if (this._tappable) {
				this.nativeViewProtected.setMovementMethod(
					android.text.method.LinkMovementMethod.getInstance()
				);
				this.nativeViewProtected.setHighlightColor(null);
			} else {
				this.nativeViewProtected.setMovementMethod(
					this._defaultMovementMethod
				);
			}
		}
	}
...
function createSpannableStringBuilder(
	...
	for (
		let i = 0,
			spanStart = 0,
			spanLength = 0,
			length = formattedString.spans.length;
		i < length;
		i++
	) {
		const span = formattedString.spans.getItem(i);
