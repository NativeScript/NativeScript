...
class UITextFieldDelegateImpl extends NSObject implements UITextFieldDelegate {
	...
	public textFieldShouldChangeCharactersInRangeReplacementString(
		...
		const owner = this._owner.get();
		if (owner) {
			return owner.textFieldShouldChangeCharactersInRangeReplacementString(
				textField,
				range,
				replacementString
			);
...
class UITextFieldImpl extends UITextField {
	...
	private _getTextRectForBounds(bounds: CGRect): CGRect {
		...
		const x = layout.toDeviceIndependentPixels(
			owner.effectiveBorderLeftWidth + owner.effectivePaddingLeft
		);
		const y = layout.toDeviceIndependentPixels(
...
export class TextField extends TextFieldBase {
	...
	initNativeView() {
		...
		this._delegate = UITextFieldDelegateImpl.initWithOwner(
			new WeakRef(this)
		);
	}
		...
		if (this.updateTextTrigger === "textChanged") {
			if (textField.secureTextEntry && this.firstEdit) {
				...
				if (range.location <= textField.text.length) {
					const newText = NSString.stringWithString(
						textField.text
	...
	[colorProperty.setNative](
		value: Color | { textColor: UIColor; tintColor: UIColor }
	) {
	...
	_updateAttributedPlaceholder(): void {
		...
		const attributedPlaceholder = NSAttributedString.alloc().initWithStringAttributes(
			stringValue,
			attributes
		);
