...
export function eachDescendant(
	view: ViewBaseDefinition,
...
export abstract class ViewBase extends Observable
	...
	public _suspendNativeUpdates(type: SuspendType): void {
		if (type) {
			this._suspendNativeUpdatesCount =
				this._suspendNativeUpdatesCount | type;
		} else {
			this._suspendNativeUpdatesCount++;
		}
	}
	public _resumeNativeUpdates(type: SuspendType): void {
		if (type) {
			...
			if (
				(this._suspendNativeUpdatesCount &
					SuspendType.IncrementalCountMask) ===
				0
			) {
	...
	public callLoaded(): void {
		this.callFunctionWithSuper(Flags.superOnLoadedCalled, () =>
			this.onLoaded()
		);
	}

	public callUnloaded(): void {
		this.callFunctionWithSuper(Flags.superOnUnloadedCalled, () =>
			this.onUnloaded()
		);
	}
	...
	public bind(
		...
		source: Object = defaultBindingSource
	): void {
	...
	public _addView(view: ViewBase, atIndex?: number) {
		...
		if (view.parent) {
			throw new Error(
				"View already has a parent. View: " +
					view +
					" Parent: " +
					view.parent
			);
	...
	public _setupUI(
		...
		if (isAndroid) {
			...
			if (nativeView) {
				...
				let result: any /* android.graphics.Rect */ = (<any>nativeView)
					.defaultPaddings;
				if (result === undefined) {
					result = org.nativescript.widgets.ViewHelper.getPadding(
						nativeView
					);
		...
		if (this.parent) {
			const nativeIndex = this.parent._childIndexToNativeChildIndex(
				atIndex
			);
...
export const classNameProperty = new Property<ViewBase, string>({
	...
	valueChanged(view: ViewBase, oldValue: string, newValue: string) {
		...
		const shouldAddModalRootViewCssClasses = cssClasses.has(
			MODAL_ROOT_VIEW_CSS_CLASS
		);
