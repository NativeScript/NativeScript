...
export namespace IOSHelper {
	...
	export function updateAutoAdjustScrollInsets(
	...
	function initLayoutGuide(controller: UIViewController) {
		...
		NSLayoutConstraint.activateConstraints(<any>[
			...
			layoutGuide.leadingAnchor.constraintEqualToAnchor(
	...
	export function layoutView(
		...
		const hasChildViewControllers =
			controller.childViewControllers.count > 0;
		if (hasChildViewControllers) {
			const fullscreen = controller.view.frame;
			position = IOSHelper.getPositionFromFrame(fullscreen);
		}

		const safeAreaWidth = layout.round(
			layout.toDevicePixels(safeAreaSize.width)
		);
		const safeAreaHeight = layout.round(
			layout.toDevicePixels(safeAreaSize.height)
		);

		const widthSpec = layout.makeMeasureSpec(safeAreaWidth, layout.EXACTLY);
		const heightSpec = layout.makeMeasureSpec(
			safeAreaHeight,
			layout.EXACTLY
		);
	...
	export function getPositionFromFrame(
		...
		const right = layout.round(
			layout.toDevicePixels(frame.origin.x + frame.size.width)
		);
	...
	export function getFrameFromPosition(
		position: { left; top; right; bottom },
		insets?: { left; top; right; bottom }
	): CGRect {
		insets = insets || { left: 0, top: 0, right: 0, bottom: 0 };

		const left = layout.toDeviceIndependentPixels(
	...
	export function shrinkToSafeArea(view: View, frame: CGRect): CGRect {
		...
		if (insets.left || insets.top) {
			...
			const adjustedFrame = IOSHelper.getFrameFromPosition(
				position,
				insets
			);
	...
	export function expandBeyondSafeArea(view: View, frame: CGRect): CGRect {
		...
		if (
			...
			inWindowPosition.right >=
				safeAreaPosition.right + fullscreenPosition.left
		) {
	...
	function getAvailableSpaceFromParent(
		view: View,
		frame: CGRect
	...
	export class UILayoutViewController extends UIViewController {
		...
		public static initWithOwner(
			...
			const controller = <UILayoutViewController>(
				UILayoutViewController.new()
			);
		...
		public viewDidLayoutSubviews(): void {
			...
			if (owner) {
				if (majorVersion >= 11) {
					...
					if (parent) {
						const parentPageInsetsTop =
							parent.nativeViewProtected.safeAreaInsets.top;
						const currentInsetsTop = this.view.safeAreaInsets.top;
						const additionalInsetsTop = Math.max(
							parentPageInsetsTop - currentInsetsTop,
							0
						);

						const parentPageInsetsBottom =
							parent.nativeViewProtected.safeAreaInsets.bottom;
						const currentInsetsBottom = this.view.safeAreaInsets
							.bottom;
						...
						if (
							additionalInsetsTop > 0 ||
							additionalInsetsBottom > 0
						) {
	...
	export class UIAdaptivePresentationControllerDelegateImp extends NSObject
		...
		public static initWithOwnerAndCallback(
			...
			const instance = <UIAdaptivePresentationControllerDelegateImp>(
				super.new()
			);
	...
	export class UIPopoverPresentationControllerDelegateImp extends NSObject
		...
		public static initWithOwnerAndCallback(
			...
			const instance = <UIPopoverPresentationControllerDelegateImp>(
				super.new()
			);
			instance.owner = owner;
			instance.closedCallback = whenClosedCallback;
