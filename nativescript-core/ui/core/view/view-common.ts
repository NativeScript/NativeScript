...
export abstract class ViewCommon extends ViewBase implements ViewDefinition {
	...
	public _handleLivesync(context?: ModuleContext): boolean {
		...
		if (
			...
			if (Trace.isEnabled()) {
				Trace.write(
					`Change Handled: Changing ${context.type} for ${this} inside ${this.page}`,
					Trace.categories.Livesync
				);
			}
	...
	_observe(
		...
		this._gestureObservers[type].push(
			gestureObserve(this, type, callback, thisArg)
		);
	}
	...
	private getModalOptions(
		args: IArguments
	): { view: ViewCommon; options: ShowModalOptions } {
		if (args.length === 0) {
			throw new Error(
	...
	protected _showNativeModalView(
		parent: ViewCommon,
	...
	protected abstract _hideNativeModalView(
		parent: ViewCommon,
	...
	public getMeasuredState(): number {
		return (
			(this._measuredWidth & layout.MEASURED_STATE_MASK) |
			((this._measuredHeight >> layout.MEASURED_HEIGHT_STATE_SHIFT) &
				(layout.MEASURED_STATE_MASK >>
	...
	public abstract onMeasure(
		widthMeasureSpec: number,
		heightMeasureSpec: number
	): void;
	public abstract onLayout(
		left: number,
		top: number,
		right: number,
		bottom: number
	): void;
	public abstract layoutNativeView(
		left: number,
		top: number,
		right: number,
		bottom: number
	...
	public static resolveSizeAndState(
		...
		return ViewHelper.resolveSizeAndState(
			size,
			specSize,
			specMode,
			childMeasuredState
	...
	public static measureChild(
		...
		return ViewHelper.measureChild(
			parent,
			child,
			widthMeasureSpec,
			heightMeasureSpec
	...
	_setCurrentLayoutBounds(
		...
		right: number,
		bottom: number
	): { boundsChanged: boolean; sizeChanged: boolean } {
		this._isLayoutValid = true;
		let boundsChanged: boolean =
			this._oldLeft !== left ||
			this._oldTop !== top ||
	...
	public getActualSize(): Size {
		...
		return {
			width: layout.toDeviceIndependentPixels(
	...
	_updateEffectiveLayoutValues(
		...
		this.effectiveWidth = PercentLength.toDevicePixels(
			style.width,
			-2,
			availableWidth
		);
		this.effectiveMarginLeft = PercentLength.toDevicePixels(
			style.marginLeft,
			0,
			availableWidth
		);
		this.effectiveMarginRight = PercentLength.toDevicePixels(
			style.marginRight,
			0,
			availableWidth
		);
		...
		const availableHeight =
			parentHeightMeasureMode === layout.UNSPECIFIED
				? -1
				: parentHeightMeasureSize;

		this.effectiveHeight = PercentLength.toDevicePixels(
			style.height,
			-2,
			availableHeight
		);
		this.effectiveMarginTop = PercentLength.toDevicePixels(
			style.marginTop,
			0,
			availableHeight
		);
		this.effectiveMarginBottom = PercentLength.toDevicePixels(
			style.marginBottom,
			0,
			availableHeight
		);
