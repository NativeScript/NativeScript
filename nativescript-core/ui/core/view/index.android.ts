...
export class View extends ViewCommon {
	...
	private hasGestureObservers() {
		return (
			...
			Object.keys(this._gestureObservers).length > 0
		);
	}
	...
	private setOnTouchListener() {
		...
		if (this.nativeViewProtected.setClickable) {
			this.nativeViewProtected.setClickable(
	...
	protected _showNativeModalView(parent: View, options: ShowModalOptions) {
		...
		if (
			options.android &&
			(<any>options).android.cancelable !== undefined
		) {
	...
	[originXProperty.setNative](value: number) {
		org.nativescript.widgets.OriginPoint.setX(
			this.nativeViewProtected,
			value
		);
	}
	...
	[originYProperty.setNative](value: number) {
		org.nativescript.widgets.OriginPoint.setY(
			this.nativeViewProtected,
			value
		);
	}
	...
	private refreshStateListAnimator() {
		...
		const duration =
			nativeView.getContext().getResources().getInteger(shortAnimTime) /
			2;
		...
		const stateListAnimator = new (<any>(
			android.animation
		)).StateListAnimator();
...
export class CustomLayoutView extends ContainerView
	...
	public _addViewToNativeVisualTree(
		...
		if (this.nativeViewProtected && child.nativeViewProtected) {
			...
			this.nativeViewProtected.addView(
