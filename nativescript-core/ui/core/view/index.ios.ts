...
export class View extends ViewCommon implements ViewDefinition {
	...
	public measure(widthMeasureSpec: number, heightMeasureSpec: number): void {
		let measureSpecsChanged = this._setCurrentMeasureSpecs(
			widthMeasureSpec,
			heightMeasureSpec
		);
		let forceLayout =
	...
	public layout(
		...
		const { boundsChanged, sizeChanged } = this._setCurrentLayoutBounds(
			left,
			top,
			right,
			bottom
	...
	public onMeasure(
		...
		let nativeHeight = 0;
		if (view) {
			const nativeSize = layout.measureNativeView(
				view,
				width,
				widthMode,
				height,
		...
		if (!CGRectEqualToRect(oldFrame, frame)) {
			if (Trace.isEnabled()) {
				Trace.write(
					this +
						" :_setNativeViewFrame: " +
						JSON.stringify(IOSHelper.getPositionFromFrame(frame)),
	...
	public _setLayoutFlags(
		...
		const { sizeChanged } = this._setCurrentLayoutBounds(
			left,
			top,
			right,
			bottom
	...
	public getSafeAreaInsets(): { left; top; right; bottom } {
		...
		if (safeAreaInsets) {
			insets.left = layout.round(
				layout.toDevicePixels(safeAreaInsets.left)
			);
			insets.top = layout.round(
				layout.toDevicePixels(safeAreaInsets.top)
			);
			insets.right = layout.round(
				layout.toDevicePixels(safeAreaInsets.right)
			);
			insets.bottom = layout.round(
				layout.toDevicePixels(safeAreaInsets.bottom)
			);
		}
	...
	public getLocationInWindow(): Point {
		...
		const pointInWindow = this.nativeViewProtected.convertPointToView(
	...
	public getLocationOnScreen(): Point {
		...
		const pointOnScreen = this.nativeViewProtected.window.convertPointToWindow(
			pointInWindow,
			null
		);
	...
	public getLocationRelativeTo(otherView: ViewDefinition): Point {
		...
		const myPointInWindow = this.nativeViewProtected.convertPointToView(
			this.nativeViewProtected.bounds.origin,
			null
		);
		const otherPointInWindow = otherView.nativeViewProtected.convertPointToView(
	...
	public updateNativeTransform() {
		...
		transform = iosNativeHelper.applyRotateTransform(
			transform,
			this.rotateX,
			this.rotateY,
			this.rotate
		...
		if (
			!CATransform3DEqualToTransform(
				this.nativeViewProtected.layer.transform,
				transform
			)
		) {
			...
			this._hasTransfrom =
				this.nativeViewProtected &&
				!CATransform3DEqualToTransform(
					this.nativeViewProtected.transform3D,
	...
	protected _showNativeModalView(parent: View, options: ShowModalOptions) {
		...
		if (!parentWithController) {
			Trace.write(
				`Could not find parent with viewController for ${parent} while showing modal view.`,
		...
		if (!controller) {
			...
			controller = <UIViewController>(
				IOSHelper.UILayoutViewController.initWithOwner(
					new WeakRef(this)
				)
			);
		...
		const animated =
			options.animated === undefined ? true : !!options.animated;
		...
		if (transitionCoordinator) {
			transitionCoordinator.animateAlongsideTransitionCompletion(
				null,
	...
	protected _hideNativeModalView(
		parent: View,
	...
	[visibilityProperty.setNative](value: Visibility) {
		switch (value) {
			...
			default:
				throw new Error(
					`Invalid visibility value: ${value}. Valid values are: "${Visibility.VISIBLE}", "${Visibility.HIDDEN}", "${Visibility.COLLAPSE}".`
				);
		}
	}
	...
	_getCurrentLayoutBounds(): {
		...
		if (nativeView && !this.isCollapsed) {
			...
			return {
				...
				bottom: Math.round(
					layout.toDevicePixels(origin.y + size.height)
				),
			};
		} else {
	...
	private _setupPopoverControllerDelegate(
		...
		const popoverPresentationController =
			controller.popoverPresentationController;
		this._popoverPresentationDelegate = IOSHelper.UIPopoverPresentationControllerDelegateImp.initWithOwnerAndCallback(
			new WeakRef(this),
			this._closeModalCallback
		);
		popoverPresentationController.delegate = <
			UIPopoverPresentationControllerDelegate
		>this._popoverPresentationDelegate;
	...
	private _setupAdaptiveControllerDelegate(controller: UIViewController) {
		this._adaptivePresentationDelegate = IOSHelper.UIAdaptivePresentationControllerDelegateImp.initWithOwnerAndCallback(
			new WeakRef(this),
			this._closeModalCallback
		);
		controller.presentationController.delegate = <
			UIAdaptivePresentationControllerDelegate
		>this._adaptivePresentationDelegate;
	}
}
