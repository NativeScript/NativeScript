...
function getProperties(property: string): Array<string> {
	...
	result = property
		.replace(parentsRegex, "parentsMatch")
		.replace(/\]/g, "")
		.split(/\.|\[/);
...
export class Binding {
	...
	private propertyChangeListeners: Map<string, Observable> = new Map<
		string,
		Observable
	>();
	...
	private update(source: any): void {
		...
		if (!types.isNullOrUndefined(source)) {
			...
			this.sourceOptions = this.resolveOptions(
				source,
	...
	): Array<{ instance: Object; property: string }> {
		let result = [];
		let currentObject = source;
		let currentObjectChanged = false;
		for (
			let i = 0, propsArrayLength = properties.length;
			i < propsArrayLength;
			i++
		) {
			...
			if (
				...
				let parentView = this.getParentView(this.target.get(), property)
					.view;
				if (parentView) {
	...
	private addPropertyChangeListeners(
		...
		sourceProperty: Array<string>,
		parentProperies?: string
	) {
		let objectsAndProperties = this.resolveObjectsAndProperties(
			source.get(),
			sourceProperty
		);
		...
		for (let i = 0, length = objectsAndProperties.length; i < length; i++) {
			...
			if (
				...
				currentObject instanceof Observable &&
	...
	private prepareExpressionForUpdate(): string {
		...
		let escapedSourceProperty = escapeRegexSymbols(
			this.options.sourceProperty
		);
		let expRegex = new RegExp(escapedSourceProperty, "g");
		let resultExp = this.options.expression.replace(
			expRegex,
	...
	private updateTwoWay(value: any) {
		...
		if (this.options.expression) {
			...
			let expressionValue = this._getExpressionValue(
				updateExpression,
				true,
				changedModel
			);
	...
	private _getExpressionValue(
		expression: string,
		isBackConvert: boolean,
		changedModel: any
	): any {
		try {
			let exp = polymerExpressions.PolymerExpressions.getExpression(
				expression
			);
			if (exp) {
				let context =
					(this.source && this.source.get && this.source.get()) ||
					global;
				let model = {};
				let addedProps = [];
	...
	public onSourcePropertyChanged(data: PropertyChangeData) {
		...
		if (changedPropertyIndex > -1) {
			...
			while (
				this.propertyChangeListeners.get(parentProps) !== data.object
			) {
		...
		if (this.options.expression) {
			const expressionValue = this._getExpressionValue(
		...
		if (
			...
			const probablyChangedObject = this.propertyChangeListeners.get(
				parentProps
			);
			if (
				probablyChangedObject &&
				probablyChangedObject !==
					data.object[sourceProps[changedPropertyIndex]]
			) {
				// remove all weakevent listeners after change, because changed object replaces object that is hooked for
				// propertyChange event
				for (
					let i = sourcePropsLength - 1;
					i > changedPropertyIndex;
					i--
				) {
	...
	private getSourcePropertyValue() {
		...
		if (this.sourceOptions) {
			...
			if (this.sourceOptions.property === bc.bindingValueKey) {
				...
				sourceOptionsInstance instanceof Observable &&
	...
	private updateTarget(value: any) {
		...
		this.updateOptions(
			this.targetOptions,
			types.isNullOrUndefined(value) ? unsetValue : value
		);
	}
	...
	private resolveOptions(
		obj: Object,
		properties: Array<string>
	): { instance: WeakRef<Object>; property: any } {
		let objectsAndProperties = this.resolveObjectsAndProperties(
			obj,
			properties
		);
		if (objectsAndProperties.length > 0) {
			...
			let prop =
				objectsAndProperties[objectsAndProperties.length - 1].property;
	...
	private updateOptions(
		options: { instance: WeakRef<any>; property: string },
		value: any
	) {
		let optionsInstance;
