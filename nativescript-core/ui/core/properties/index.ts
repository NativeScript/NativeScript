...
export interface CssPropertyOptions<T extends Style, U>
	extends PropertyOptions<T, U> {
...
export interface ShorthandPropertyOptions<P> {
	...
	readonly converter: (
		value: string | P
	) => [CssProperty<any, any> | CssAnimationProperty<any, any>, any][];
...
export function _evaluateCssVariableExpression(
	view: ViewBase,
	cssName: string,
	value: string
): string {
	...
	while (lastValue !== output) {
		...
		output = `${output.substring(
			0,
			idx
		)}${cssVariableValue}${output.substring(endIdx + 1)}`;
	}
...
function getPropertiesFromMap(
	map
): Property<any, any>[] | CssProperty<any, any>[] {
	const props = [];
	Object.getOwnPropertySymbols(map).forEach((symbol) =>
		props.push(map[symbol])
	);
...
export class Property<T extends ViewBase, U>
	implements TypedPropertyDescriptor<U>, Property<T, U> {
	...
	constructor(options: PropertyOptions<T, U>) {
		...
		this.set = function (this: T, boxedValue: U): void {
			...
			const changed: boolean = equalityComparer
		...
		this.nativeValueChange = function (owner: T, value: U): void {
			...
			const changed = equalityComparer
				? !equalityComparer(oldValue, value)
				: oldValue !== value;
			if (changed) {
				owner[key] = value;
...
export class InheritedProperty<T extends ViewBase, U> extends Property<T, U>
	implements InheritedProperty<T, U> {
			...
			function (value: U): void {
				...
				if (currentValue !== newValue) {
					...
					that.eachChild((child) => {
						const childValueSource =
							child[sourceKey] || ValueSource.Default;
						if (reset) {
...
export class CssProperty<T extends Style, U> implements CssProperty<T, U> {
	...
	constructor(options: CssPropertyOptions<T, U>) {
		...
		function setCssValue(this: T, newValue: U | string): void {
			...
			const currentValueSource: number =
...
export class CssAnimationProperty<T extends Style, U>
	...
	constructor(options: CssAnimationPropertyOptions<T, U>) {
		...
		const setNative = (this.setNative = Symbol(
			propertyName + ":setNative"
		));
		const eventName = propertyName + "Change";
		...
		function descriptor(
			...
			return {
				...
				set(this: T, boxedValue: U | string) {
					...
					const reset =
						boxedValue === unsetValue || boxedValue === "";
					...
					if (
						view[setNative] &&
						(computedValueChanged || isSet !== wasSet)
					) {
						if (view._suspendNativeUpdatesCount) {
							if (view._suspendedUpdates) {
								view._suspendedUpdates[propertyName] = property;
		...
		const defaultPropertyDescriptor = descriptor(
			defaultValueKey,
			ValueSource.Default,
			false,
			false,
		...
		const cssPropertyDescriptor = descriptor(
			cssValue,
			ValueSource.Css,
			false,
			false,
		...
		const stylePropertyDescriptor = descriptor(
			styleValue,
			ValueSource.Local,
			true,
			true,
		...
		const keyframePropertyDescriptor = descriptor(
			keyframeValue,
			ValueSource.Keyframe,
			false,
			false,
			false
		);
...
export class InheritedCssProperty<T extends Style, U> extends CssProperty<T, U>
	implements InheritedCssProperty<T, U> {
	...
	constructor(options: CssPropertyOptions<T, U>) {
		...
		const setFunc = (valueSource: ValueSource) =>
			function (this: T, boxedValue: any): void {
				...
				if (!view) {
					Trace.write(
						`${boxedValue} not set to view's property because ".viewRef" is cleared`,
						Trace.categories.Style,
						Trace.messageType.warn
					);
				...
				if (reset) {
					...
					if (
						valueSource === ValueSource.Css &&
						currentValueSource === ValueSource.Local
					) {
						return;
					}
				} else {
				...
				if (changed) {
					...
					view.eachChild((child) => {
						...
						const childValueSource =
							childStyle[sourceKey] || ValueSource.Default;
						if (reset) {
							if (childValueSource === ValueSource.Inherited) {
...
export class ShorthandProperty<T extends Style, P>
	implements ShorthandProperty<T, P> {
	private registered: boolean;
...
function inheritablePropertyValuesOn(
	view: ViewBase
): Array<{ property: InheritedProperty<any, any>; value: any }> {
...
function inheritableCssPropertyValuesOn(
	style: Style
): Array<{ property: InheritedCssProperty<any, any>; value: any }> {
	const array = new Array<{
...
type PropertyInterface =
	| Property<ViewBase, any>
	| CssProperty<Style, any>
	| CssAnimationProperty<Style, any>;

export const initNativeView = profile(
	'"properties".initNativeView',
	function initNativeView(view: ViewBase): void {
		if (view._suspendedUpdates) {
...
export function propagateInheritableProperties(
	view: ViewBase,
	child: ViewBase
): void {
...
export function propagateInheritableCssProperties(
	parentStyle: Style,
	childStyle: Style
): void {
	const inheritableCssPropertyValues = inheritableCssPropertyValuesOn(
		parentStyle
	);
