...
function initializeNativeClasses() {
		...
		public onDestroyView() {
			...
			if (hasRemovingParent && this.owner.selectedIndex === this.index) {
				...
				if (thisView) {
					...
					if (
						thisViewParent &&
						thisViewParent instanceof android.view.ViewGroup
					) {
	...
	class BottomNavigationBarImplementation extends org.nativescript.widgets
		.BottomNavigationBar {
		constructor(
			context: android.content.Context,
			public owner: BottomNavigation
		) {
			super(context);
...
function setElevation(
	bottomNavigationBar: org.nativescript.widgets.BottomNavigationBar
) {
...
export class BottomNavigation extends TabNavigationBase {
	...
	public createNativeView() {
		...
		// CONTENT VIEW
		const contentView = new org.nativescript.widgets.ContentLayout(
			this._context
		);
	...
	private getDrawableInfo(image: android.graphics.Bitmap): IconInfo {
		if (image) {
			...
			let imageDrawable = new android.graphics.drawable.BitmapDrawable(
	...
	private setIconColor(tabStripItem: TabStripItem, color?: Color) {
		const tabBarItem = this._bottomNavigationBar.getViewForItemAt(
			tabStripItem._index
		);
