	...
	public tabBarControllerShouldSelectViewController(
		...
		if (owner) {
			...
			if (tabBarController.viewControllers) {
				const position = tabBarController.viewControllers.indexOfObject(
					viewController
				);
...
export class BottomNavigation extends TabNavigationBase {
	...
	private setIconColor(
		...
		if (
			forceReload ||
			(!this._unSelectedItemColor && !this._selectedItemColor)
		) {
			...
			if (this.tabStrip && this.tabStrip.items) {
				this.tabStrip.items.forEach((item) => {
					...
					if (this._selectedItemColor && item.nativeView) {
						item.nativeView.selectedImage = this.getIcon(
							item,
							this._selectedItemColor
						);
	...
	private setViewAttributes(item: UITabBarItem, view: View): any {
		...
		if (!this._selectedItemColor && !this._unSelectedItemColor) {
			...
			this.viewController.tabBar.unselectedItemTintColor =
				this._unSelectedItemColor && this._unSelectedItemColor.ios;
			this.viewController.tabBar.selectedImageTintColor =
				this._selectedItemColor && this._selectedItemColor.ios;
		}
		...
		if (
			!this.viewController.tabBar.barTintColor &&
