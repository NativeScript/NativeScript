...
export function getValidTime(
	...
	if (picker.minuteInterval > 1) {
		...
		minute =
			minuteFloor +
			(minute === minuteFloor + 1 ? picker.minuteInterval : 0);
...
function getMinMaxTimeErrorMessage(picker: TimePickerDefinition): string {
	return `Min time: (${toString(picker.minHour)}:${toString(
		picker.minMinute
	)}), max time: (${toString(picker.maxHour)}:${toString(picker.maxMinute)})`;
}
...
function getErrorMessage(
	...
	return `${propertyName} property value (${toString(newValue)}:${toString(
		picker.minute
	)}) is not valid. ${getMinMaxTimeErrorMessage(picker)}.`;
}

@CSSType("TimePicker")
export abstract class TimePickerBase extends View
	implements TimePickerDefinition {
	public hour: number;
...
export const minuteIntervalProperty = new Property<TimePickerBase, number>({
	...
	valueChanged: (picker, oldValue, newValue) => {
		if (!isMinuteIntervalValid(newValue)) {
			throw new Error(
				getErrorMessage(picker, "minuteInterval", newValue)
			);
		}
	},
	valueConverter: (v) => parseInt(v),
