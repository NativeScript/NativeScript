...
> Note: the `Details` button will be available when the execution of the test complete and there is at least one failing test.

When you click on the button, you will be redirected to the report's page. On the left pane are visualized all failed jobs.

> Note: Each item name consists of the application name, type of device and platform version: `pr-e2e-tests-[application-name]-[device-type]-[platform-version]`. Usually, applications included in PRs are part of NativeScript repository and most probably are part of the plugin repository.

Base on the executed suite, one of the following or all of the following files will be generated: `mochawesome.html` | `index.html` | `unit-tests.log`. Some of the reports also might include `*.png`, `*.logs` or `[page source].xml` files that can help in understanding where is the problem.

For example:
1. When we select the `index.html` page, an additional `TestNG Results` sidebar will be displayed. There we will find a list of all failures. 
2. When we select one of them, you will see on the right side all tests, that have been executed. The problematic ones will be coloured in red. 
